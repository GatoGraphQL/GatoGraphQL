...
var args={pageSection:pageSection,newDOMs:newDOMs};t.extendArgs(args,options);popJSLibraryManager.execute('pageSectionNewDOMsBeforeInitialize',args);t.initPageSectionBranches(pageSection,newDOMs,options);var settings=t.getFetchPageSectionSettings(pageSection);if(settings.activeLinks){t.activeLinks(newDOMs);}
popJSLibraryManager.execute('pageSectionNewDOMsInitialized',args);},initPageSectionBranches:function(pageSection,newDOMs,options){var t=this;t.runPageSectionJSMethods(pageSection,options);var jsSettings=popManager.getPageSectionJsSettings(pageSection);var blockBranches=jsSettings['initjs-blockbranches'];if(blockBranches){t.initBlockBranches(pageSection,$(blockBranches.join(',')).not('.'+M.JS_INITIALIZED),options);}
...
return elem.closest('data-templateid');},updateDocument:function(){var t=this;var feedback=t.getTopLevelFeedback();t.documentTitle=unescapeHtml(feedback[M.URLPARAM_TITLE]);document.title=t.documentTitle;},activeLinks:function(elems){var t=this;var feedback=t.getTopLevelFeedback();var parentPageId=feedback[M.URLPARAM_PARENTPAGEID];if(parentPageId){elems.find('.menu-item-object-id-'+parentPageId).each(function(){var menuItem=$(this);menuItem.addClass('active');menuItem.parents('.menu-item').addClass('active');jQuery(document).ready(function($){var collapse=menuItem.closest('.collapse.pop-showactive').not('.in');collapse.collapse('show');});});}},hideMessages:function(pageSection){var t=this;var status=popPageSectionManager.getPageSectionStatus(pageSection);var loading=status.find('.pop-loading');var error=status.find('.pop-error');loading.addClass('hidden');error.addClass('hidden');},executeSetFilterBlockParams:function(pageSection,block,filter){var t=this;var blockParams=t.getBlockParams(pageSection,block);blockParams.filter=filter.find('.'+M.FILTER_INPUT).filter(function(){return $.trim(this.value);}).serialize();if(blockParams.filter){blockParams.filter+='&'+filter.find('.'+M.FILTER_NAME_INPUT).serialize();}},setFilterBlockParams:function(pageSection,block,filter){var t=this;if(t.jsInitialized(block)){t.executeSetFilterBlockParams(pageSection,block,filter);}
...
;(function($){popPrettyPrint={prettyPrint:function(args){var t=this;var targets=args.targets;targets.find('.prettyprint').addBack('.prettyprint').each(function(e){var elem=$(this);elem.html(prettyPrintOne(elem.html()));});},};})(jQuery);popJSLibraryManager.register(popPrettyPrint,['prettyPrint']);
