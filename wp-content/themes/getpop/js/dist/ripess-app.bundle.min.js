...
return str;});Handlebars.registerHelper('img',function(imageData,options){var url=options.hash.url,title=options.hash.title||"",alt=options.hash.alt||"",classs=options.hash.class||"",ret="";if(url){ret+='<a href="'+url+'" title="'+title+'">';}
...
return mempage[pssId][targetId];},isFirstLoad:function(pageSection){var t=this;return t.firstLoad[popManager.getSettingsId(pageSection)];},init:function(){var t=this;t.initDocument();t.initTopLevelJson();popUserAccount.initialLogin();var memory=t.getMemory();$.each(memory.settings.configuration,function(pssId,psConfiguration){var psId=psConfiguration[M.JS_FRONTENDID];var pageSection=$('#'+psId);pageSection.attr('data-settings-id',pssId);pageSection.addClass('pop-pagesection');t.initPageSectionSettings(pageSection,psConfiguration);t.addPageSectionIds(pageSection,psConfiguration[M.JS_TEMPLATE]);t.initPageSection(pageSection);t.renderPageSection(pageSection);t.pageSectionInitialized(pageSection);});popBrowserHistory.replaceState(t.getTopLevelFeedback()[M.URLPARAM_URL]);t.updateDocument();t.documentInitialized();t.backgroundLoad(M.BACKGROUND_LOAD);t.backgroundLoad(t.getTopLevelFeedback()[M.URLPARAM_BACKGROUNDLOADURLS]);$(document).ready(function($){$('body').removeClass('pop-loading-initial');});},expandJSKeys:function(context){var t=this;if(context&&M.COMPACT_JS_KEYS){if(context[M.JS_MODULES]){context.modules=context[M.JS_MODULES];}
...
return elem.closest('data-templateid');},updateDocument:function(){var t=this;var feedback=t.getTopLevelFeedback();t.documentTitle=unescapeHtml(feedback[M.URLPARAM_TITLE]);document.title=t.documentTitle;$('.menu-item').removeClass('active');var parentPageId=feedback[M.URLPARAM_PARENTPAGEID];if(parentPageId){$('.menu-item-object-id-'+parentPageId).each(function(){var menuItem=$(this);menuItem.addClass('active');menuItem.parents('.menu-item').addClass('active');jQuery(document).ready(function($){var collapse=menuItem.closest('.collapse.pop-showactive').not('.in');collapse.collapse('show');});});}},hideMessages:function(pageSection){var t=this;var status=popPageSectionManager.getPageSectionStatus(pageSection);var loading=status.find('.pop-loading');var error=status.find('.pop-error');loading.addClass('hidden');error.addClass('hidden');},executeSetFilterBlockParams:function(pageSection,block,filter){var t=this;var blockParams=t.getBlockParams(pageSection,block);blockParams.filter=filter.find('.'+M.FILTER_INPUT).filter(function(){return $.trim(this.value);}).serialize();if(blockParams.filter){blockParams.filter+='&'+filter.find('.'+M.FILTER_NAME_INPUT).serialize();}},setFilterBlockParams:function(pageSection,block,filter){var t=this;if(t.jsInitialized(block)){t.executeSetFilterBlockParams(pageSection,block,filter);}
else{block.one('js:initialized',function(){t.executeSetFilterBlockParams(pageSection,block,filter);});}},setFilterParams:function(pageSection,filter){var t=this;var block=t.getBlock(filter);t.setFilterBlockParams(pageSection,block,filter);},filter:function(pageSection,filter){var t=this;var block=t.getBlock(filter);t.setFilterParams(pageSection,filter);t.reload(pageSection,block);},setReloadBlockParams:function(pageSection,block){var t=this;var blockParams=t.getBlockParams(pageSection,block);if(blockParams[M.DATALOAD_PARAMS]&&blockParams[M.DATALOAD_PARAMS][M.URLPARAM_PAGED]){blockParams[M.DATALOAD_PARAMS][M.URLPARAM_PAGED]='';}
...
$.ajax({dataType:"json",url:fetchUrl,data:postData,beforeSend:function(jqXHR,settings){jqXHR.url=settings.url;params.url[jqXHR.url]=url;params.loading.push(url);t.handleLoadingStatus(pageSection,'add',options);},complete:function(jqXHR){var fetchedUrl=params.url[jqXHR.url];delete params.url[jqXHR.url];params.loading.splice(params.loading.indexOf(fetchedUrl),1);t.handleLoadingStatus(pageSection,'remove',options);if(options['urlfetched-callbacks']){var handler='urlfetched:'+escape(fetchedUrl);$.each(options['urlfetched-callbacks'],function(index,callback){$(document).one(handler,callback);});}
...
$.ajax({dataType:"json",url:fetchUrl,data:post_data,type:type,beforeSend:function(jqXHR,settings){jqXHR.url=settings.url;params.url[jqXHR.url]=loadingUrl;params.loading.push(loadingUrl);params.operation[loadingUrl]=options.operation;params.action[loadingUrl]=options.action;params['paramsscope-url'][loadingUrl]=block.data('paramsscope-url');if(options.reload){params.reload.push(loadingUrl);}},success:function(response,textStatus,jqXHR){t.backgroundLoad(response.feedback.toplevel[M.URLPARAM_BACKGROUNDLOADURLS]);if(options['hide-latestcount']){block.children('.blocksection-latestcount').find('.pop-latestcount').addClass('hidden').find('.pop-count').text('0');}
...
var tags=itemObject['tags'];if(tags&&tags.length){var tagssep=selector+'.tag';targets.push(tagssep+tags.join(tagssep));}
jQuery.each(itemObject['authors'],function(index,author){var target=selector+'.author'+author;if(cats&&cats.length){target+='.author-'+post_type+'-'+cats.join('.author-'+post_type+'-');}
...
var carousels=newDOMs.find('.carousel.pop-bscomponent').addBack('.carousel.pop-bscomponent');if(carousels.length){t.initComponentHandlers(pageSection,carousels,'slid.bs.carousel');}},initComponentHandlers:function(pageSection,targets,handler){var t=this;targets.on(handler,function(e){var component=$(this);var blocks=$(component.data('initjs-targets'));var block_data={'post-data':component.data('post-data'),}
t.recursiveSetBlockData(pageSection,blocks,block_data);});targets.one(handler,function(e){var component=$(this);var blocks=$(component.data('initjs-targets')).not('.'+M.JS_INITIALIZED);var options={'force-init':true};var url=component.data('intercept-url');if(url){options.url=url;}
popManager.initBlockBranches(pageSection,blocks,options);});targets.on(handler,function(e){var component=$(this);var blocks=$(component.data('initjs-targets'));if(component.data('onetime-refetch')){t.recursiveOneTimeRefetch(pageSection,blocks);}
...
if(direction=='up'){popManager.historyReplaceState(target);}},context:opts.context});t.destroy(pageSection,block,waypoint);});});},waypointsTheater:function(args){var t=this;var pageSection=args.pageSection,block=args.block,targets=args.targets;jQuery(document).ready(function($){targets.each(function(){var target=$(this);var opts=t.getOptions(pageSection,target);var waypoint=new Waypoint({element:target,handler:function(direction){var waypoint=this;var target=this.element;if(popManager.isHidden(target)){return;}
...
else{t.reEnable(pageSection,block,waypoint);}});});},getOptions:function(pageSection,waypoint){var t=this;var options={};var context=pageSection.filter('.pop-waypoints-context');if(context.length){options.context=context;}
else{context=waypoint.closest('.pop-waypoints-context');if(context.length){options.context=context;}}
return options;},destroy:function(pageSection,block,waypoint){var t=this;var pageSectionPage=popManager.getPageSectionPage(block);pageSectionPage.one('destroy',function(){waypoint.destroy();});},};})(jQuery);popJSLibraryManager.register(popWaypoints,['documentInitialized','waypointsFetchMore','waypointsTheater','waypointsHistoryStateOriginal','waypointsHistoryStateNew']);;(function($){popWindow={windowSize:function(args){var t=this;var pageSection=args.pageSection,targets=args.targets;targets.click(function(e){e.preventDefault();var control=$(this);var target=$(control.data('target'));if(target.length){var action=control.data('toggle');if(action=='window-fullsize'){t.fullsize(target);}
...
cookie.children("button.close").click(function(e){e.preventDefault();var cookie=$(this).parent('.cookie');$.cookie(cookie.data('cookieid'),"dismissed",{expires:90,path:"/"});});});},cookieCollapse:function(args){var t=this;var pageSection=args.pageSection,targets=args.targets;targets.each(function(){var container=$(this);var cookieid=container.data('cookieid');var collapse=$(container.data('cookiecollapse'));if(!($.cookie(cookieid))){collapse.addClass('in');}
if(container.data('deletecookiebtn')){$(container.data('deletecookiebtn')).click(function(e){$.cookie(cookieid,null,{expires:-1,path:"/"});});}
if(container.data('setcookiebtn')){$(container.data('setcookiebtn')).click(function(e){$.cookie(cookieid,"dismissed",{expires:180,path:"/"});});}});},sortable:function(args){var t=this;var pageSection=args.pageSection,block=args.block,targets=args.targets;targets.sortable({cursor:"move"});},onClickThenClick:function(args){var t=this;var targets=args.targets;targets.each(function(){var clickable=$(this);var trigger=$(clickable.data('triggertarget'));trigger.click(function(e){clickable.trigger('click');});});},embedPreviewFromInput:function(pageSection,block,connector){var t=this;var input=$(connector.data('input-target'));var url=input.val();if(url&&isUrlValid(url)){var iframe=$(connector.data('iframe-target'));var merged=t.embedPreview(pageSection,block,iframe,url);connector.data('iframe-target','#'+merged.targetContainer.attr('id'));}},getURLParamInputValue:function(block,input){var t=this;var val,urlparam=input.data('urlparam');if(urlparam){var url=block.data('paramsscope-url');val=getParam(urlparam,url);}
...
val=val||'';input.val(val);},fillURLInput:function(input,link){var t=this;link=popManager.getOriginalLink(link);var url=t.getUrl(link);input.val(url);},getUrl:function(link){var t=this;link=popManager.getOriginalLink(link);var url=link.data('target-url');if(!url){var blockTarget=$(link.data('blocktarget'));var pageSection=popManager.getPageSection(blockTarget);url=popManager.getBlockFilteringUrl(pageSection,blockTarget);if(!url){url=window.location.href;}}
return url;},execReplaceCode:function(input,url){var t=this;var placeholder=input.data('code-placeholder');var code=placeholder.format(url);input.val(code);},};})(jQuery);popJSLibraryManager.register(popFunctions,['expandJSKeys','showmore','hideEmpty','imageResponsive','printWindow','copyHeader','fillURLParamInput','fillAddonInput','fillModalInput','fillAddonURLInput','fillModalURLInput','switchTargetClass','doNothing','fetchMore','fetchMoreDisable','saveLastClicked','retrySendRequest','highlight','replaceCode','modalReloadEmbedPreview','reloadEmbedPreview','embedCode','socialmedia','mediaplayer','reset','browserUrl','smallScreenHideCollapse','cookies','cookieCollapse','sortable','onClickThenClick','beep']);;(function($){popBlockFunctions={hideIfEmptyBlock:function(args){var t=this;var pageSection=args.pageSection,block=args.block;if(popManager.hideIfEmpty(pageSection,block)){block.addClass('hidden');}
else{block.removeClass('hidden');}}};})(jQuery);popJSLibraryManager.register(popBlockFunctions,['hideIfEmptyBlock']);;(function($){popControls={controlOpenAll:function(args){var t=this;var pageSection=args.pageSection,block=args.block,targets=args.targets;targets.click(function(e){e.preventDefault();var control=$(this);var targetBlock=$(control.data('blocktarget'));var target=control.data('target');var url=popManager.getBlockFilteringUrl(pageSection,targetBlock);popManager.fetch(url,{target:target});});},controlPrint:function(args){var t=this;var pageSection=args.pageSection,block=args.block,targets=args.targets;targets.click(function(e){e.preventDefault();var control=$(this);var url=t.getBlockFilteringUrl(pageSection,control);url=popManager.getPrintUrl(url);popSystem.openPrint(url);});},controlSocialMedia:function(args){var t=this;var pageSection=args.pageSection,block=args.block,targets=args.targets;targets.click(function(e){e.preventDefault();var control=$(this);var url=t.getBlockFilteringUrl(pageSection,control);var provider=control.data('provider');var settings=M.SOCIALMEDIA[provider];var shareUrl=settings['share-url'].replace(new RegExp('%url%','g'),encodeURIComponent(url)).replace(new RegExp('%title%','g'),encodeURIComponent(popManager.documentTitle));popSystem.openSocialMedia(shareUrl);});},getBlockFilteringUrl:function(pageSection,control){var t=this;var targetBlock=$(control.data('blocktarget'));return popManager.getBlockFilteringUrl(pageSection,targetBlock);},};})(jQuery);popJSLibraryManager.register(popControls,['controlOpenAll','controlPrint','controlSocialMedia']);
;/*
