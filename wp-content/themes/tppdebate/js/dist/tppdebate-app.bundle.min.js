...
else{block.one('js:initialized',function(){t.executeSetFilterBlockParams(pageSection,block,filter);});}},filter:function(pageSection,block,filter){var t=this;t.setFilterBlockParams(pageSection,block,filter);t.reload(pageSection,block);t.blockScrollTop(pageSection,block);},blockScrollTop:function(pageSection,block){var t=this;var modal=block.closest('.modal');if(modal.length==0){t.scrollToElem(pageSection,block,true);}
...
else{glyphicon.addClass('glyphicon-fullscreen').removeClass('glyphicon-resize-small');}});targets.click(function(e){e.preventDefault();var button=$(this);var fullScreen=button.closest('.pop-fullscreen');fullScreen.toggleFullScreen();});},newWindow:function(args){var t=this;var targets=args.targets;targets.click(function(e){e.preventDefault();window.open(popManager.getUnembedUrl(window.location.href),'_blank');});},clickURLParam:function(args){var t=this;var url=getParam(M.URLPARAM_URL);if(url){$(document).ready(function($){popManager.click(decodeURIComponent(url));});}},initDelegatorFilter:function(args){var t=this;var targets=args.targets;targets.submit(function(e){e.preventDefault();var form=$(this);if(form.data('blocktarget')){var block=$(form.data('blocktarget'));var pageSection=popManager.getPageSection(block);if(popManager.isBlockGroup(block)){t.filterBlockGroup(pageSection,block,form);}
else{popManager.filter(pageSection,block,form);}}});},initBlockFilter:function(args){var t=this;var pageSection=args.pageSection,block=args.block,targets=args.targets;targets.submit(function(e){e.preventDefault();var form=$(this);if(form.data('blocktarget')){block=$(form.data('blocktarget'));pageSection=popManager.getPageSection(block);}
