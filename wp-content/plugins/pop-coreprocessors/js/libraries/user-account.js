...
window.popUserAccount = {
	...
	loadAvatar : function(args) {
		...
		targets.attr('src', that.sessions[domain].avatar);
	},
	...
	fetchAllDomainsLoggedInUserData : function(options) {
		...
		$.each(M.USERLOGGEDIN_DATA_PAGEURLS, function(index, url) {
			...
			if (that.sessions[domain].initialuserdata) {
				...
				// that.initialuserdata is a flag so we identify the first log in. Needed for not reloading data initially that must not be reloaded. Eg: TPP Debate Create OpinionatedVoted block. (It reloads its own data initially)
	...
	updateUserInfo : function(domain, id, name, avatar, url, roles, userattributes) {
		...
		if (that.sessions[domain].id != id || that.sessions[domain].avatar != avatar) {

			that.sessions[domain].id = id;
			that.sessions[domain].avatar = avatar;
		...
		var eq_roles = $(that.sessions[domain].roles).not(roles).length === 0 && $(roles).not(that.sessions[domain].roles).length === 0;
		if (!eq_roles) {

			that.sessions[domain].roles = roles;
			...
			$.each(that.sessions[domain].roles, function(index, role) {
		...
		var eq_userattributes = $(that.sessions[domain].userattributes).not(userattributes).length === 0 && $(userattributes).not(that.sessions[domain].userattributes).length === 0;
		if (!eq_userattributes) {

			that.sessions[domain].userattributes = userattributes;
			...
			$.each(that.sessions[domain].userattributes, function(index, userattribute) {
	...
	loginout : function(source, domain, loggedin, id, name, avatar, url, roles, userattributes) {
		...
		that.updateUserInfo(domain, id, name, avatar, url, roles, userattributes);
	...
	feedbackLoginOut : function(domain, initial) {
		...
		if (typeof userInfo != 'undefined') {
			...
			that.loginout(source, domain, loggedin, id, name, avatar, url, roles, userattributes);
