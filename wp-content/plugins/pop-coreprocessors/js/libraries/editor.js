...
popEditor = {
	...
	editor : function(args) {
		...
		// The execution comes loaded from doing wp_editor($value, $editor_id, $options); in 'editor-formcomponents-base.php'
		...
		if (!POP_CACHED_SETTINGS && popManager.isFirstLoad(pageSection)) {
			return;
		}
		$(document).ready(function($){
		
			// var editor = elem.find('.pop-editor');
			targets.each(function() {

				// The editor is wrapped with a div as to add the {{generateId}}, so the real editor is its child
				var editor = $(this).find('.pop-editor');
				var editorId = editor.attr('id');
				var settings = tinyMCEPreInit.mceInit[M.TEMPLATE_EDITOR_NAME];
				...
				var qtOptions = $.extend({}, tinyMCEPreInit.qtInit[M.TEMPLATE_EDITOR_NAME], {id: editorId});
				quicktags(qtOptions);
...
// Initialize
//-------------------------------------------------
popJSLibraryManager.register(popEditor, ['editor']);
