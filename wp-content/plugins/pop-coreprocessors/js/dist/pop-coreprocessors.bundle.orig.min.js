...
return addspinner;},onlineOffline:function(){var t=this;window.addEventListener('online',t.checkOnlineOffline);window.addEventListener('offline',t.checkOnlineOffline);t.checkOnlineOffline();},checkOnlineOffline:function(){var t=this;if(navigator.onLine){$('#body').addClass('online').removeClass('offline');}else{$('#body').addClass('offline').removeClass('online');}},links:function(){var t=this;var imgRegex=new RegExp(/\.(gif|jpg|jpeg|tiff|png)$/i);var otherRegex=new RegExp(/\.(pdf|css|js|zip|tar|ppt|pptx|doc|docx|xls|xlsx)$/i);var allowedAnchors=[];$.each(M.ALLOWED_URLS,function(index,domain){allowedAnchors.push('a[href^="'+domain+'"]');});$(document).on('click',allowedAnchors.join(','),function(e){var anchor=$(this);var url=anchor.attr('href');if(imgRegex.test(url)){var method='linksImage';if(popJSLibraryManager.getLibraries(method).length){e.preventDefault();var args={anchor:anchor}
...
return false;},};})(jQuery);popJSLibraryManager.register(popPerfectScrollbar,['scrollbarHorizontal','scrollbarVertical','getPosition','scrollTop']);;(function($){popFunctions={expandJSKeys:function(args){var t=this;var context=args.context;if(context[M.JS_FONTAWESOME]){context.fontawesome=context[M.JS_FONTAWESOME];}
