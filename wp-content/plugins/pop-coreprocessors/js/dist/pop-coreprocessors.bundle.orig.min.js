...
popManager.maybeRedirect(blockFeedback);});},clearInput:function(args){var t=this;var pageSection=args.pageSection,targets=args.targets;targets.each(function(){var input=$(this);input.closest('form').on('clear',function(){input.val('');});});},makeAlwaysRefetchBlock:function(args){var t=this;var pageSection=args.pageSection,block=args.block;block.on('visible',function(){var block=$(this);t.refetchBlock(pageSection,block);});},getDestroyPageTabParams:function(pageSection,link){var t=this;return{url:link.data('url')||popManager.getBlock(link).data('paramsscope-url'),target:link.attr('target')||popManager.getFrameTarget(pageSection)};},filterBlockGroup:function(pageSection,blockGroup,filter){var t=this;var activeBlock=popManager.getBlockGroupActiveBlock(blockGroup);var blocks=popManager.getBlockGroupBlocks(blockGroup);blocks.each(function(){var block=$(this);popManager.setFilterBlockParams(pageSection,block,filter);if(block.attr('id')==activeBlock.attr('id')){popManager.reload(pageSection,block);}
...
var eq_roles=$(t.roles).not(roles).length===0&&$(roles).not(t.roles).length===0;if(!eq_roles){t.roles=roles;$.each(M.ROLES,function(index,role){$(document.body).removeClass(role);});$.each(t.roles,function(index,role){$(document.body).addClass(role);});updates.roles=t.roles;}
var eq_userattributes=$(t.userattributes).not(userattributes).length===0&&$(userattributes).not(t.userattributes).length===0;if(!eq_userattributes){t.userattributes=userattributes;$.each(M.USERATTRIBUTES,function(index,userattribute){$(document.body).removeClass(userattribute);});$.each(t.userattributes,function(index,userattribute){$(document.body).addClass(userattribute);});updates.userattributes=t.userattributes;}
if(!$.isEmptyObject(updates)){updates.id=id;$(document).triggerHandler('user:updated',[updates]);}},loginout:function(source,loggedin,id,name,avatar,url,roles,userattributes){var t=this;t.updateUserInfo(id,name,avatar,url,roles,userattributes);if(t.loggedin!=loggedin){t.loggedin=loggedin;var name,body=$(document.body);if(t.loggedin){body.data('loggedinuser-id',id).addClass('loggedin user-'+id);name='user:loggedin';}
...
var template=iframe.data('templateid');popJSRuntimeManager.setBlockURL(block.data('toplevel-url'));var merged=popManager.mergeTargetTemplate(pageSection,block,template,options);popManager.runJSMethods(pageSection,block,template,'full');var iframeid=popJSRuntimeManager.getLastGeneratedId(popManager.getSettingsId(pageSection),popManager.getSettingsId(block),template);block.data('embed-iframe','#'+iframeid);return merged;},fillInput:function(input,link){var t=this;link=popManager.getOriginalLink(link);var val,attr=input.data('attr');if(attr){val=link.data(attr);}
