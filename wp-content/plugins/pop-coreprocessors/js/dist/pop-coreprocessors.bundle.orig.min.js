...
targets.push(target);});return new Handlebars.SafeString(targets.join(','));});Handlebars.registerHelper('formatFeedbackMessage',function(message,options){var topLevelSettings=options.hash.tls,pageSectionSettings=options.hash.pss,blockSettings=options.hash.bs;message=popSystem.formatFeedbackMessage(message,topLevelSettings,pageSectionSettings,blockSettings);return new Handlebars.SafeString(message);});;(function($){popSystem={documentInitialized:function(){var t=this;t.links();t.onlineOffline();},activeLinks:function(args){var t=this;var pageSection=args.pageSection,targets=args.targets,domain=args.domain;var settings=popManager.getFetchPageSectionSettings(pageSection);if(settings.activeLinks){var feedback=popManager.getTopLevelFeedback(domain);var parentPageId=feedback[M.URLPARAM_PARENTPAGEID];if(parentPageId){targets.find('.menu-item-object-id-'+parentPageId).each(function(){var menuItem=$(this);menuItem.addClass('active');menuItem.parents('.menu-item').addClass('active');});}}},fullscreen:function(args){var t=this;var targets=args.targets;$(document).bind("fullscreenchange",function(){var glyphicon=targets.find('.glyphicon');if($(document).fullScreen()){glyphicon.removeClass('glyphicon-fullscreen').addClass('glyphicon-resize-small');}
else{glyphicon.addClass('glyphicon-fullscreen').removeClass('glyphicon-resize-small');}});targets.click(function(e){e.preventDefault();var button=$(this);var fullScreen=button.closest('.pop-fullscreen');fullScreen.toggleFullScreen();});},newWindow:function(args){var t=this;var targets=args.targets;targets.click(function(e){e.preventDefault();window.open(popManager.getUnembedUrl(window.location.href),'_blank');});},initDelegatorFilter:function(args){var t=this;var delegatorPageSection=args.pageSection,targets=args.targets;targets.submit(function(e){e.preventDefault();var form=$(this);if(form.data('blocktarget')){var block=$(form.data('blocktarget'));var pageSection=popManager.getPageSection(block);if(popManager.isBlockGroup(block)){t.filterBlockGroup(pageSection,block,form);}
...
popManager.filter(pageSection,block,form);});},initBlockGroupFilter:function(args){var t=this;var pageSection=args.pageSection,blockGroup=args.block,targets=args.targets;targets.each(function(e){var filter=$(this);blocks.each(function(){var block=$(this);popManager.setFilterBlockParams(pageSection,block,filter);});});targets.submit(function(e){e.preventDefault();var filter=$(this);t.filterBlockGroup(pageSection,blockGroup,filter);});},reloadBlockGroup:function(args){var t=this;var pageSection=args.pageSection,blockGroup=args.block,targets=args.targets;targets.click(function(e){e.preventDefault();var activeBlock=popManager.getBlockGroupActiveBlock(blockGroup);popManager.reload(pageSection,activeBlock);});},reloadBlock:function(args){var t=this;var pageSection=args.pageSection,block=args.block,targets=args.targets;targets.click(function(e){e.preventDefault();popManager.reload(pageSection,block);});},loadLatestBlock:function(args){var t=this;var domain=args.domain,pageSection=args.pageSection,block=args.block,targets=args.targets;targets.click(function(e){e.preventDefault();popManager.loadLatest(domain,pageSection,block);});},timeoutLoadLatestBlock:function(args){var t=this;var domain=args.domain,pageSection=args.pageSection,block=args.block,targets=args.targets;targets.each(function(){var target=$(this);var time=target.data('clicktime')||30000;t.execTimeoutLoadLatestBlock(domain,pageSection,block,target,time);});},displayBlockDatasetCount:function(args){var t=this;var domain=args.domain,pageSection=args.pageSection,block=args.block;if(block.data('datasetcount-target')){var jsSettings=popManager.getJsSettings(domain,pageSection,block);var when=jsSettings['display-datasetcount-when']||['oncreated'];var updateTitle=jsSettings['datasetcount-updatetitle'];if(when.indexOf('oncreated')>-1){$(document).ready(function($){var target=$(block.data('datasetcount-target'));popManager.displayDatasetCount(domain,pageSection,block,target,updateTitle);});}
if(when.indexOf('onrendered')>-1){block.on('rendered',function(e){var target=$(block.data('datasetcount-target'));popManager.displayDatasetCount(domain,pageSection,block,target,updateTitle);});}}},clearDatasetCount:function(args){var t=this;var pageSection=args.pageSection,block=args.block,targets=args.targets;targets.each(function(){var link=$(this);var target=$(link.data('datasetcount-target'));var updateTitle=link.data('datasetcount-updatetitle')?true:false;link.click(function(e){e.preventDefault();t.execClearDatasetCount(target,updateTitle);});link.hover(function(){t.execClearDatasetCount(target,updateTitle);});});},clearDatasetCountOnUserLoggedOut:function(args){var t=this;var pageSection=args.pageSection,block=args.block,targets=args.targets;targets.each(function(){var link=$(this);var target=$(link.data('datasetcount-target'));var updateTitle=link.data('datasetcount-updatetitle')?true:false;$(document).on('user:loggedout',function(e){t.execClearDatasetCount(target,updateTitle);});});},onDestroyPageSwitchTab:function(args){var t=this;var pageSection=args.pageSection,targets=args.targets;targets.each(function(){var link=$(this);var pageSectionPage=popManager.getPageSectionPage(link);pageSectionPage.on('destroy',function(){t.switchTab(link);})})},addOpenTab:function(args){var t=this;var pageSection=args.pageSection,targets=args.targets,addOpenTab=args.addOpenTab;if(addOpenTab===false){return;}
targets.each(function(){var link=$(this);var params=t.getDestroyPageTabParams(pageSection,link);popManager.addOpenTab(params.url,params.target);});},closePageTab:function(args){var t=this;var pageSection=args.pageSection,targets=args.targets;targets.click(function(e){e.preventDefault();var link=$(this);var params=t.getDestroyPageTabParams(pageSection,link);popManager.triggerDestroyTarget(params.url,params.target);});},resetOnSuccess:function(args){var t=this;var domain=args.domain,pageSection=args.pageSection,targets=args.targets;var pssId=popManager.getSettingsId(pageSection);targets.on('fetched',function(e,response){var block=$(this);var bsId=popManager.getSettingsId(block);var blockFeedback=response.feedback.block[pssId][bsId];if(blockFeedback.result===true){popManager.reset(domain,pageSection,block,{'skip-restore':true});}});},resetOnUserLogout:function(args){var t=this;var domain=args.domain,pageSection=args.pageSection,targets=args.targets;$(document).on('user:loggedout:'+domain,function(e,source){targets.each(function(){var block=$('#'+$(this).attr('id'));if(block.length){popManager.reset(domain,pageSection,block);}});});},destroyPageOnSuccess:function(args){var t=this;var pageSection=args.pageSection,targets=args.targets;var pssId=popManager.getSettingsId(pageSection);targets.on('fetched',function(e,response){var block=$(this);var bsId=popManager.getSettingsId(block);var blockFeedback=response.feedback.block[pssId][bsId];var target=popManager.getFrameTarget(pageSection);if(blockFeedback.result===true){var destroyTime=block.data('destroytime')||0;setTimeout(function(){popManager.triggerDestroyTarget(popManager.getTargetParamsScopeURL(block),target);},destroyTime);}});},destroyPageOnUserLoggedOut:function(args){var t=this;var domain=args.domain,pageSection=args.pageSection,targets=args.targets;pageSection.one('completed',function(){t.execDestroyPageOnUserLoggedOut(domain,targets);});},destroyPageOnUserNoRole:function(args){var t=this;var domain=args.domain,pageSection=args.pageSection,targets=args.targets;pageSection.one('completed',function(){t.execDestroyPageOnUserNoRole(domain,pageSection,targets);});},refetchBlockOnUserLoggedIn:function(args){var t=this;var domain=args.domain,pageSection=args.pageSection,targets=args.targets;$(document).one('user:loggedin:'+domain,function(e,source){if(source=='initialfeedback'){return;}
t.execRefetchBlock(pageSection,targets);});},nonendingRefetchBlockOnUserLoggedIn:function(args){var t=this;var domain=args.domain,pageSection=args.pageSection,targets=args.targets;t.execNonendingRefetchBlockOnUserEvent(pageSection,targets,'user:loggedin:'+domain);},nonendingRefetchBlockOnUserLoggedInOut:function(args){var t=this;var domain=args.domain,pageSection=args.pageSection,targets=args.targets;t.execNonendingRefetchBlockOnUserEvent(pageSection,targets,'user:loggedinout:'+domain);},deleteBlockFeedbackValueOnUserLoggedInOut:function(args){var t=this;var domain=args.domain,pageSection=args.pageSection,block=args.block;$(document).on('user:loggedinout:'+domain,function(e,source){if(source=='initialfeedback'||source=='initialuserdata'){return;}
var jsSettings=popManager.getJsSettings(domain,pageSection,block);var keys=jsSettings['user:loggedinout-deletefeedbackvalue']||[];if(keys.length){var blockFeedback=popManager.getBlockFeedback(domain,pageSection,block);$.each(keys,function(index,keyLevels){var feedbackLevel=blockFeedback;for(i=0;i<(keyLevels.length)-1;i++){feedbackLevel=feedbackLevel[keyLevels[i]];}
delete feedbackLevel[keyLevels[(keyLevels.length)-1]];});}});},scrollTopOnUserLoggedInOut:function(args){var t=this;var domain=args.domain,pageSection=args.pageSection,block=args.block,targets=args.targets;$(document).on('user:loggedinout:'+domain,function(e,source){if(source=='initialfeedback'||source=='initialuserdata'){return;}
targets.each(function(){var target=$(this);if($('#'+target.attr('id')).length){var jsSettings=popManager.getJsSettings(domain,pageSection,block,target);var animate=jsSettings['scrolltop-animate']||false;popManager.scrollToElem(target,target,animate);}});});},refetchBlockGroupOnUserLoggedIn:function(args){var t=this;var domain=args.domain,pageSection=args.pageSection,targets=args.targets;$(document).one('user:loggedin:'+domain,function(e,source){if(source=='initialfeedback'){return;}
...
if(blockFeedback['show-msgs']&&blockFeedback['msgs']&&blockFeedback['msgs'].length){popManager.scrollToElem(pageSection,block.children('.blocksection-messagefeedback').first(),true);}
popManager.maybeRedirect(blockFeedback);});},clearInput:function(args){var t=this;var pageSection=args.pageSection,targets=args.targets;targets.each(function(){var input=$(this);input.closest('form').on('clear',function(){input.val('');});});},makeAlwaysRefetchBlock:function(args){var t=this;var pageSection=args.pageSection,block=args.block;block.on('visible',function(){var block=$(this);t.refetchBlock(pageSection,block);});},addDomainClass:function(args){var t=this;var domain=args.domain,pageSection=args.pageSection,block=args.block,targets=args.targets;targets.each(function(){var target=$(this);var jsSettings=popManager.getJsSettings(domain,pageSection,block,target);var prefix=jsSettings['prefix']||'';targets.addClass(prefix+getDomainId(domain));});},formatFeedbackMessage:function(message,topLevelSettings,pageSectionSettings,blockSettings){var t=this;var args={message:message,tls:topLevelSettings,pss:pageSectionSettings,bs:blockSettings};popJSLibraryManager.execute('formatFeedbackMessage',args);return args.message;},getDestroyPageTabParams:function(pageSection,link){var t=this;return{url:link.data('url')||popManager.getTargetParamsScopeURL(popManager.getBlock(link)),target:link.attr('target')||popManager.getFrameTarget(pageSection)};},filterBlockGroup:function(pageSection,blockGroup,filter){var t=this;var activeBlock=popManager.getBlockGroupActiveBlock(blockGroup);var blocks=popManager.getBlockGroupBlocks(blockGroup);blocks.each(function(){var block=$(this);popManager.setFilterBlockParams(pageSection,block,filter);if(block.attr('id')==activeBlock.attr('id')){popManager.reload(pageSection,block);}
else{t.makeOneTimeRefetch(pageSection,block);}});},execClearDatasetCount:function(target,updateTitle){var t=this;target.html('').addClass('hidden');if(updateTitle){document.title=popManager.documentTitle;}},execTimeoutLoadLatestBlock:function(domain,pageSection,block,target,time){var t=this;var jsSettings=popManager.getJsSettings(domain,pageSection,block,target);var options={'skip-status':true,'datasetcount-target':jsSettings['datasetcount-target'],'datasetcount-updatetitle':jsSettings['datasetcount-updatetitle'],};var onlyLoggedIn=jsSettings['only-loggedin']||false;if(onlyLoggedIn&&!popUserAccount.isLoggedIn(domain)){$(document).one('user:loggedin:'+domain,function(e,source){t.execTimeoutLoadLatestBlock(domain,pageSection,block,target,time);});return;}
setTimeout(function(){if($('#'+target.attr('id')).length){popManager.loadLatest(domain,pageSection,block,options);t.execTimeoutLoadLatestBlock(domain,pageSection,block,target,time);}},time);},timeoutClosePageSection:function(pageSection,closeTime){var t=this;setTimeout(function(){popPageSectionManager.close(pageSection);popManager.closeMessageFeedbacks(pageSection);},closeTime);},execNonendingRefetchBlockOnUserEvent:function(pageSection,targets,handler){var t=this;$(document).on(handler,function(e,source){if(source=='initialfeedback'||source=='initialuserdata'){return;}
t.execRefetchBlock(pageSection,targets);});},execRefetchBlock:function(pageSection,blocks){var t=this;blocks.each(function(){var block=$('#'+$(this).attr('id'));if(block.length){t.refetchBlock(pageSection,block);}});},execDestroyPageOnUserLoggedOut:function(domain,targets){var t=this;$(document).one('user:loggedout:'+domain,function(e,source){targets.each(function(){var block=$('#'+$(this).attr('id'));if(block.length){var pageSection=popManager.getPageSection(block);var target=popManager.getFrameTarget(pageSection);popManager.triggerDestroyTarget(popManager.getTargetParamsScopeURL(block),target);}});});},execDestroyPageOnUserNoRole:function(domain,pageSection,block){var t=this;var neededRole=block.data('neededrole');$(document).one('user:updated:'+domain,function(e,updates){block=$('#'+block.attr('id'));if(block.length){if(updates.roles&&updates.roles.indexOf(neededRole)==-1){var target=popManager.getFrameTarget(pageSection);popManager.triggerDestroyTarget(popManager.getTargetParamsScopeURL(block),target);}
else{t.execDestroyPageOnUserNoRole(domain,pageSection,block);}}});},switchTab:function(link){var t=this;if(link.prev('a.pop-pagetab-btn').hasClass('active')){var current=popManager.getPageSectionPage(link);var next=current.nextAll('.pop-pagesection-page').first();if(!next.length){next=current.prevAll('.pop-pagesection-page').first();}
if(next.length){next.find('a.pop-pagetab-btn').trigger('click');}}},replicateMultipleTopLevel:function(pageSection,targets){var t=this;targets.click(function(e){e.preventDefault();var link=$(this);var addUniqueId=link.data('unique-url')||false;t.execReplicateTopLevel(pageSection,link,true,addUniqueId);});},replicateSingleTopLevel:function(pageSection,targets){var t=this;targets.one('click',function(e){e.preventDefault();var link=$(this);t.execReplicateTopLevel(pageSection,link,false,false);});},execReplicateTopLevel:function(pageSection,link,generateUniqueId,addUniqueId){var t=this;var url=link.data('original-url');var domain=getDomain(url);var memory=popManager.getMemory(domain);var interceptUrl=link.data('intercept-url');var target=popManager.getFrameTarget(pageSection);var replicableMemory=popManager.getReplicableMemory(interceptUrl,target);var tlFeedback=popManager.getTopLevelFeedback(domain);$.each(replicableMemory.feedback.toplevel,function(key,value){if($.type(value)=='array'&&value.length==0){}
else if($.type(value)=='object'){if(!tlFeedback[key]){tlFeedback[key]={};}
$.extend(tlFeedback[key],value);}
else{tlFeedback[key]=value;}});if(generateUniqueId){if(addUniqueId&&url.indexOf('#')>-1){tlFeedback[M.UNIQUEID]=url.substr(url.indexOf('#')+1);}
else{popManager.generateUniqueId(domain);if(addUniqueId){url=popManager.addUniqueId(url);}}}
var title=link.data('title');tlFeedback[M.URLPARAM_TITLE]=title;tlFeedback[M.URLPARAM_PARENTPAGEID]=null;tlFeedback[M.URLPARAM_URL]=url;popManager.maybeUpdateDocument(domain,pageSection);},replicateMultiplePageSection:function(pageSection,targets){var t=this;targets.click(function(e){e.preventDefault();var link=$(this);t.execReplicatePageSection(pageSection,link);});},replicateSinglePageSection:function(pageSection,targets){var t=this;targets.one('click',function(e){e.preventDefault();var link=$(this);t.execReplicatePageSection(pageSection,link);});},execReplicatePageSection:function(pageSection,link){var t=this;var bsId=link.data('block-settingsid');var pssId=popManager.getSettingsId(pageSection);var template=link.data('templateid');var interceptUrl=link.data('intercept-url');var domain=getDomain(interceptUrl);var memory=popManager.getMemory(domain);var target=popManager.getFrameTarget(pageSection);var replicableMemory=popManager.getReplicableMemory(interceptUrl,target);$.extend(memory.feedback.pagesection,replicableMemory.feedback.pagesection);$.each(replicableMemory.feedback.block,function(ipssId,ipsFeedback){$.extend(memory.feedback.block[ipssId],ipsFeedback);$.extend(memory.dataset[ipssId],replicableMemory.dataset[ipssId]);$.extend(memory['query-state'].general[ipssId],replicableMemory['query-state'].general[ipssId]);$.extend(memory['query-state'].domain[ipssId],replicableMemory['query-state'].domain[ipssId]);});var tlFeedback=popManager.getTopLevelFeedback(domain);var psFeedback=popManager.getPageSectionFeedback(domain,pageSection);var psConfiguration=popManager.getPageSectionConfiguration(domain,pageSection);if(!psFeedback['intercept-urls'][template]){psFeedback['intercept-urls'][template]={};}
...
blockSettingsIds[settingElem.data('key')]=val;});psConfiguration[M.JS_BLOCKSETTINGSIDS]=blockSettingsIds;popManager.initPageSectionSettings(domain,pageSection,psConfiguration);var options={'js-args':{relatedTarget:link},url:tlFeedback[M.URLPARAM_URL]}
popManager.renderPageSection(domain,pageSection,options);},refetchBlock:function(pageSection,block){var t=this;var options={'post-data':block.data('post-data'),'show-disabled-layer':true};popManager.refetch(pageSection,block,options);},makeOneTimeRefetch:function(pageSection,block){var t=this;if(!popManager.jsInitialized(block)||!popManager.isContentLoaded(pageSection,block)){return;}
...
return addspinner;},onlineOffline:function(){var t=this;window.addEventListener('online',t.checkOnlineOffline);window.addEventListener('offline',t.checkOnlineOffline);t.checkOnlineOffline();},checkOnlineOffline:function(){var t=this;if(navigator.onLine){$(document.body).addClass('online').removeClass('offline');}else{$(document.body).addClass('offline').removeClass('online');}},links:function(){var t=this;var imgRegex=new RegExp(/\.(gif|jpg|jpeg|tiff|png)$/i);var otherRegex=new RegExp(/\.(pdf|css|js|zip|tar|ppt|pptx|doc|docx|xls|xlsx)$/i);var allowedAnchors=[];$.each(M.ALLOWED_DOMAINS,function(index,domain){allowedAnchors.push('a[href^="'+domain+'"]');});$(document).on('click',allowedAnchors.join(','),function(e){var anchor=$(this);var url=anchor.attr('href');if(imgRegex.test(url)){var method='linksImage';if(popJSLibraryManager.getLibraries(method).length){e.preventDefault();var args={anchor:anchor}
...
if(popManager.targetExists(target)){e.preventDefault();if(url==M.HOME_DOMAIN){url=M.HOME_DOMAIN+'/';}
...
if(url){var args={url:url,delimiter:delimiter,query:query};popJSLibraryManager.execute('mentionsSource',args);url=args.url;var previousData=t.previousData[delimiter]||[];process(previousData);t.fetchData(delimiter,url,process);}}},render:function(item){var t=this;var delimiter=t.infereDelimiter(item);var html=popManager.getHtml(M.MENTIONS.TYPES[delimiter].template,item);return'<li><a href="javascript:;"><span>'+html+'</span></a></li>';},insert:function(item){var t=this;var delimiter=t.infereDelimiter(item);return'<span>'+delimiter+item[M.MENTIONS.TYPES[delimiter].key]+'</span>&nbsp;';},infereDelimiter:function(item){var t=this;var ret='';$.each(M.MENTIONS.TYPES,function(delimiter,settings){if(typeof item[settings.key]!='undefined'){ret=delimiter;return-1;}});return ret;},};})(jQuery);popJSLibraryManager.register(popMentions,['documentInitialized']);function mentions_source(query,process,delimiter){popMentions.source(query,process,delimiter);}
function mentions_insert(item){return popMentions.insert(item);}
function mentions_render(item){return popMentions.render(item);};(function($){popFeaturedImage={frame:null,documentInitialized:function(args){var t=this;t.frame=wp.media({title:M.LABELS.MEDIA_FEATUREDIMAGE_TITLE,multiple:false,library:{type:'image'},button:{text:M.LABELS.MEDIA_FEATUREDIMAGE_BTN}});t.frame.on('open',function(){var featuredImage=t.getOpenFeaturedImage();var selected=featuredImage.find('input[type="hidden"]');if(selected.val()){var selection=t.frame.state().get('selection');attachment=wp.media.attachment(selected.val());attachment.fetch();selection.add(attachment?[attachment]:[]);}});t.frame.on('close',function(){var featuredImage=$('.pop-featuredimage.open');var configuration={};var selection=t.frame.state().get('selection');if(selection.length){var value,imgsrc;var imgSize=featuredImage.data('img-size');selection.map(function(attachment){attachment=attachment.toJSON();var imgData=attachment.sizes[imgSize]||attachment.sizes['medium']||attachment.sizes['thumbnail'];value=attachment.id;imgsrc={src:imgData.url,width:imgData.width,height:imgData.height};});var block=popManager.getBlock(featuredImage);var pageSection=popManager.getPageSection(block);var domain=popManager.getBlockTopLevelDomain(block);t.merge(domain,pageSection,block,featuredImage,value,imgsrc);}
featuredImage.removeClass('open');});},featuredImageSet:function(args){var t=this;var pageSection=args.pageSection,block=args.block,targets=args.targets;targets.click(function(e){e.preventDefault();var button=$(this);var featuredImage=button.closest('.pop-featuredimage');featuredImage.addClass('open');t.frame.open();});},featuredImageRemove:function(args){var t=this;var domain=args.domain,pageSection=args.pageSection,block=args.block,targets=args.targets;targets.click(function(e){e.preventDefault();var button=$(this);var featuredImage=button.closest('.pop-featuredimage');t.remove(domain,pageSection,block,featuredImage);});},merge:function(domain,pageSection,block,featuredImage,value,imgsrc){var t=this;var options={extendContext:{value:value,img:imgsrc}}
var template=featuredImage.data('merge-template');popJSRuntimeManager.setBlockURL(block);popManager.mergeTargetTemplate(domain,pageSection,block,template,options);popManager.runJSMethods(domain,pageSection,block,template,'full');},remove:function(domain,pageSection,block,featuredImage){var t=this;t.merge(domain,pageSection,block,featuredImage,'','');},getFrame:function(){var t=this;return t.frame;},getOpenFeaturedImage:function(){var t=this;return $('.pop-featuredimage.open');}};})(jQuery);popJSLibraryManager.register(popFeaturedImage,['documentInitialized','featuredImageSet','featuredImageRemove']);;(function($){popMediaManager={domain:null,initialized:{},needsRefresh:{},documentInitialized:function(){var t=this;$(document).on('uploader:start:settings',function(e,settings){t.setUploadSettings(settings);});$(document.body).on('click.add-media-button','.insert-media',function(e){var elem=$(e.currentTarget),editor=$('#'+elem.data('editor'));var block=popManager.getBlock(editor);t.domain=popManager.getBlockTopLevelDomain(block);});$(document).on('user:loggedout',function(e){t.needsRefresh={featuredImage:true,'editor-gallery':true,'editor-insert':true};});$(document).ready(function($){$(document.body).on('click.add-media-button','.insert-media,.media-menu-item',function(e){var state=wp.media.frame.state();var states=['gallery','insert'];if(states.indexOf(state.id)>-1){if(t.initialized['editor-'+state.id]&&t.needsRefresh['editor-'+state.id]){t.refresh(state);}
t.initialized['editor-'+state.id]=true;t.needsRefresh['editor-'+state.id]=false;}});popFeaturedImage.getFrame().on('open',function(){if(t.initialized.featuredImage&&t.needsRefresh.featuredImage){t.refresh(wp.media.frame.state());}
t.initialized.featuredImage=true;t.needsRefresh.featuredImage=false;});});},featuredImageSet:function(args){var t=this;var domain=args.domain,targets=args.targets;targets.click(function(e){t.domain=domain;});},getDomain:function(){var t=this;return t.domain;},refresh:function(state){var t=this;state.get('library')._requery(true);},setUploadSettings:function(settings){var t=this;if(settings.multipart_params.post_id){delete settings.multipart_params.post_id;}}};})(jQuery);popJSLibraryManager.register(popMediaManager,['documentInitialized']);popJSLibraryManager.register(popMediaManager,['featuredImageSet'],true);;(function($){popMediaManagerCORS={domains:{'editor-gallery':null,'editor-insert':null,featuredImage:null},documentInitialized:function(){var t=this;$(document).on('wp.ajax:send:options',function(e,options){t.setEditorOptions(options);});$(document).on('uploader:start:settings',function(e,settings){t.setUploadSettings(settings);});$(document).ready(function($){$(document.body).on('click.add-media-button','.insert-media,.media-menu-item',function(e){var domain=popMediaManager.getDomain();var state=wp.media.frame.state();var states=['gallery','insert'];if(states.indexOf(state.id)>-1){if(domain&&t.domains['editor-'+state.id]&&domain!=t.domains['editor-'+state.id]){popMediaManager.needsRefresh['editor-'+state.id]=true;}
t.domains['editor-'+state.id]=domain;}});popFeaturedImage.getFrame().on('open',function(){var domain=popMediaManager.getDomain();if(domain&&t.domains.featuredImage&&domain!=t.domains.featuredImage){popMediaManager.needsRefresh.featuredImage=true;}
t.domains.featuredImage=domain;});});},setEditorOptions:function(options){var t=this;var domain=popMediaManager.getDomain();if(domain){if(domain!=M.HOME_DOMAIN){options.url=domain+M.AJAXURL;options.xhrFields={withCredentials:true};options.crossDomain=true;}
if(options.data&&options.data.action){var action_nonces={'query-attachments':'media-form','save-attachment-compat':'media-form','save-attachment-order':'media-form','send-attachment-to-editor':'media-send-to-editor','send-link-to-editor':'media-send-to-editor',};var nonce_key=action_nonces[options.data.action];if(nonce_key){var nonce=popManager.getTopLevelFeedback(domain)[M.URLPARAM_NONCES][nonce_key];if(options.data._wpnonce){options.data._wpnonce=nonce;}
if(options.data.nonce){options.data.nonce=nonce;}
if(options.data.query&&options.data.query._wpnonce){options.data.query._wpnonce=nonce;}
if(options.data.query&&options.data.query.nonce){options.data.query.nonce=nonce;}}}}},setUploadSettings:function(settings){var t=this;var domain=popMediaManager.getDomain();if(domain){settings.url=domain+M.UPLOADURL;var nonce_keys={'upload-attachment':'media-form'}
var action=settings.multipart_params.action;var nonce_key=nonce_keys[action];if(nonce_key){settings.multipart_params._wpnonce=popManager.getTopLevelFeedback(domain)[M.URLPARAM_NONCES][nonce_key];}}}};})(jQuery);popJSLibraryManager.register(popMediaManagerCORS,['documentInitialized'],true);;(function($){popTabs={documentInitialized:function(args){var t=this;var show=false;var currentURL=popManager.getTabsCurrentURL();var tabs=popManager.getScreenOpenTabs();$.each(tabs,function(target,urls){if((urls.length>1)||(target!=M.URLPARAM_TARGET_MAIN&&urls.length)||(target==M.URLPARAM_TARGET_MAIN&&urls[0]!=currentURL)){show=true;return-1;}});if(show){if($.cookie('opentabs')){if($.cookie('opentabs')=="true"){popManager.openTabs();}
else{popManager.keepScreenOpenTab(popManager.getTabsCurrentURL(),M.URLPARAM_TARGET_MAIN);}}
else{var status=$('#'+M.IDS_APPSTATUS);var param1='popTabs.openTabs()';var param2='popTabs.keepScreenOpenTab(\'{0}\', \'{1}\')'.format(popManager.getTabsCurrentURL(),M.URLPARAM_TARGET_MAIN);status.prepend(M.TABS_REOPENMSG.format(param1,param2));}}},saveCookie:function(value){var t=this;if($('#'+M.IDS_TABS_REMEMBERCHECKBOX).prop('checked')){$.cookie('opentabs',value,{expires:365,path:"/"});}},openTabs:function(){var t=this;t.saveCookie("true");popManager.openTabs();},keepScreenOpenTab:function(url,target){var t=this;t.saveCookie("false");popManager.keepScreenOpenTab(url,target);}};})(jQuery);popJSLibraryManager.register(popTabs,['documentInitialized']);;(function($){popUserAccount={sessions:{},initDomain:function(args){var t=this;var domain=args.domain;t.sessions[domain]={initialuserdata:true,loggedin:false,id:null,name:'',avatar:'',url:'',roles:[],userattributes:[],}},documentInitialized:function(args){var t=this;var domain=args.domain;t.checkpointInvalidateBlocks();t.fetchAllDomainsLoggedInUserData();},pageSectionInitialized:function(args){var t=this;var pageSection=args.pageSection;pageSection.on('fetched',function(e,options,url,domain){t.feedbackLoginOut(domain,false);});},blockInitialized:function(args){var t=this;var pageSection=args.pageSection,block=args.block;block.on('fetched',function(e,response,action,domain){t.feedbackLoginOut(domain,false);});},loadAvatar:function(args){var t=this;var domain=args.domain,targets=args.targets;targets.attr('src',t.sessions[domain].avatar);},initialLogin:function(domain){var t=this;feedbackAvailable=t.feedbackLoginOut(domain,true);t.sessions[domain].initialuserdata=!feedbackAvailable;},fetchAllDomainsLoggedInUserData:function(options){var t=this;$.each(M.USERLOGGEDIN_DATA_PAGEURLS,function(index,url){var domain=getDomain(url);if(t.sessions[domain].initialuserdata){t.fetchLoggedInUserData(url,options);}});},fetchLoggedInUserData:function(url,options){var t=this;options=options||{};options['loadingmsg-target']=M.USERLOGGEDIN_LOADINGMSG_TARGET;options.silentDocument=true;popManager.fetch(url,options);},isLoggedInAnyDomain:function(){var t=this;var ret=false;$.each(t.sessions,function(domain,state){if(state.loggedin){ret=true;return-1;}});return ret;},isLoggedIn:function(domain){var t=this;return t.sessions[domain].loggedin;},updateUserInfo:function(domain,id,name,avatar,url,roles,userattributes){var t=this;var updates={};var domainId=getDomainId(domain);if(t.sessions[domain].name!=name){t.sessions[domain].name=name;$('.pop-user-name.'+domainId).text(name);updates.name=name;}
if(t.sessions[domain].id!=id||t.sessions[domain].avatar!=avatar){t.sessions[domain].id=id;t.sessions[domain].avatar=avatar;$('.pop-user-avatar.'+domainId).attr('src',avatar);updates.avatar=avatar;}
if(t.sessions[domain].url!=url){t.sessions[domain].url=url;$('.pop-user-url.'+domainId).attr('href',url);updates.url=url;}
var eq_roles=$(t.sessions[domain].roles).not(roles).length===0&&$(roles).not(t.sessions[domain].roles).length===0;if(!eq_roles){t.sessions[domain].roles=roles;$.each(M.ROLES,function(index,role){$(document.body).removeClass(role+'-'+domainId);});$.each(t.sessions[domain].roles,function(index,role){$(document.body).addClass(role+'-'+domainId);});updates.roles=roles;}
var eq_userattributes=$(t.sessions[domain].userattributes).not(userattributes).length===0&&$(userattributes).not(t.sessions[domain].userattributes).length===0;if(!eq_userattributes){t.sessions[domain].userattributes=userattributes;$.each(M.USERATTRIBUTES,function(index,userattribute){$(document.body).removeClass(userattribute+'-'+domainId);});$.each(t.sessions[domain].userattributes,function(index,userattribute){$(document.body).addClass(userattribute+'-'+domainId);});updates.userattributes=userattributes;}
if(!$.isEmptyObject(updates)){updates.id=id;$(document).triggerHandler('user:updated',[updates,domain]);$(document).triggerHandler('user:updated:'+domain,[updates]);}},loginout:function(source,domain,loggedin,id,name,avatar,url,roles,userattributes){var t=this;t.updateUserInfo(domain,id,name,avatar,url,roles,userattributes);if(t.sessions[domain].loggedin!=loggedin){t.sessions[domain].loggedin=loggedin;var extraClasses=[];if(domain==M.HOME_DOMAIN){extraClasses.push('loggedin-localdomain');}
var name,body=$(document.body);var domainId=getDomainId(domain);if(t.sessions[domain].loggedin){extraClasses.push('loggedin-anydomain');body.data('loggedinuser-id-'+domainId,id).addClass('loggedin-'+domainId+' user-'+id+'-'+domainId);$.each(extraClasses,function(index,classs){body.addClass(classs);});name='user:loggedin';}
else{if(!t.isLoggedInAnyDomain()){extraClasses.push('loggedin-anydomain');}
body.removeClass('loggedin-'+domainId).removeClass('user-'+body.data('loggedinuser-id-'+domainId)+'-'+domainId).data('loggedinuser-id-'+domainId,'');$.each(extraClasses,function(index,classs){body.removeClass(classs);});name='user:loggedout';}
if(source!='initialfeedback'){t.closeCheckpointMessages(domain);}
$(document).triggerHandler(name,[source,domain]);$(document).triggerHandler(name+':'+domain,[source]);$(document).triggerHandler('user:loggedinout',[source,domain]);$(document).triggerHandler('user:loggedinout:'+domain,[source]);}},feedbackLoginOut:function(domain,initial){var t=this;var topLevelFeedback=popManager.getTopLevelFeedback(domain);var userInfo=topLevelFeedback[M.DATALOAD_USER];if(typeof userInfo!='undefined'){var loggedin,id,name,avatar,url;loggedin=userInfo[M.DATALOAD_USER_LOGGEDIN];id=userInfo[M.DATALOAD_USER_ID];name=userInfo[M.DATALOAD_USER_NAME];avatar=userInfo[M.DATALOAD_USER_AVATAR];url=userInfo[M.DATALOAD_USER_URL];roles=userInfo[M.DATALOAD_USER_ROLES];userattributes=userInfo[M.DATALOAD_USER_ATTRIBUTES];var source='feedback';if(initial){source='initialfeedback';}
else if(t.sessions[domain].initialuserdata){source='initialuserdata';t.sessions[domain].initialuserdata=false;}
t.loginout(source,domain,loggedin,id,name,avatar,url,roles,userattributes);return true;}
return false;},checkpointInvalidateBlocks:function(){var t=this;$(document).on('user:loggedin',function(e,source,domain){if(source=='initialfeedback'||source=='initialuserdata'){return;}
t.closeCheckpointMessages(domain);});},closeCheckpointMessages:function(domain){var t=this;jQuery(document).ready(function($){var domainId=getDomainId(domain);$('.alert.in.pop-messagefeedback.checkpoint.'+domainId).removeClass('fade').alert('close');});},getLoggedInUserId:function(domain){var t=this;if(t.sessions[domain]&&t.sessions[domain].loggedin){return t.sessions[domain].id;}
return false;}};})(jQuery);popJSLibraryManager.register(popUserAccount,['initDomain','documentInitialized','pageSectionInitialized','blockInitialized','loadAvatar']);;(function($){popWaypoints={waypointsHistoryStateNew:function(args){var t=this;var pageSection=args.pageSection,block=args.block,targets=args.targets;jQuery(document).ready(function($){targets.each(function(){var target=$(this);var opts=t.getOptions(pageSection,target);var waypointUp=new Waypoint({element:target,handler:function(direction){var waypoint=this;var target=this.element;if(popManager.isHidden(target)){return;}
...
if(direction=='up'){popManager.historyReplaceState(target);}},context:opts.context});t.destroy(pageSection,block,waypoint);});});},waypointsToggleClass:function(args){var t=this;var pageSection=args.pageSection,block=args.block,targets=args.targets;jQuery(document).ready(function($){targets.each(function(){var target=$(this);var recipient=$(target.data('target'));var classs=target.data('class');if(recipient.length&&classs){var opts=t.getOptions(pageSection,target);var waypoint=new Waypoint({element:target,handler:function(direction){var waypoint=this;var target=this.element;if(popManager.isHidden(target)){return;}
if(direction=='down'){recipient.addClass(classs);}
else if(direction=='up'){recipient.removeClass(classs);}},context:opts.context,offset:opts.context?0:function(){return t.getOffset(this.element);}});t.destroy(pageSection,block,waypoint);}});});},waypointsToggleCollapse:function(args){var t=this;var targets=args.targets;jQuery(document).ready(function($){targets.each(function(){var collapse=$(this);var target=$(collapse.data('collapse-target'));if(target.length){var block=popManager.getBlock(target);var pageSection=popManager.getBlock(block);t.execWaypointsToggleCollapse(pageSection,block,target,collapse);}});});},waypointsTheater:function(args){var t=this;var pageSection=args.pageSection,block=args.block,targets=args.targets;jQuery(document).ready(function($){targets.each(function(){var target=$(this);var opts=t.getOptions(pageSection,target);var waypoint=new Waypoint({element:target,handler:function(direction){var waypoint=this;var target=this.element;if(popManager.isHidden(target)){return;}
if(direction=='down'){popPageSectionManager.theater(true);}
else if(direction=='up'){popPageSectionManager.theater(false);}},context:opts.context,offset:opts.context?0:function(){return t.getOffset(this.element);}});t.destroy(pageSection,block,waypoint);});});},waypointsFetchMore:function(args){var t=this;var pageSection=args.pageSection,block=args.block,targets=args.targets;jQuery(document).ready(function($){var waypoints=[];var contentLoaded=popManager.isContentLoaded(pageSection,block);targets.each(function(){var target=$(this);var opts=t.getOptions(pageSection,target);var waypoint=new Waypoint({element:target,handler:function(direction){var waypoint=this;var target=this.element;if(direction=='down'){if(popManager.isHidden(target)){return;}
target.trigger('click');}},context:opts.context,enabled:contentLoaded,offset:'bottom-in-view'});t.destroy(pageSection,block,waypoint);waypoints.push(waypoint);});block.on('fetchCompleted',function(e){var block=$(this);if(popManager.stopFetchingBlock(pageSection,block)){$.each(waypoints,function(index,waypoint){if(waypoint.enabled){waypoint.disable();t.reEnable(pageSection,block,waypoint);}});}
else{$.each(waypoints,function(index,waypoint){if(!waypoint.enabled){waypoint.enable();}
waypoint.context.refresh();});}});});},reEnable:function(pageSection,block,waypoint){var t=this;block.one('beforeReload',function(){block.one('fetchCompleted',function(){if(!popManager.stopFetchingBlock(pageSection,block)){waypoint.enable();}
...
daterange.on('apply.daterangepicker',function(ev,picker){$(this).val(picker.startDate.format(picker.locale.format)+' - '+picker.endDate.format(picker.locale.format));});daterange.daterangepicker(daterange_settings,t.callback);});},callback:function(start,end){var t=this;var dateinput=t.element;dateinput.siblings('.from').val(start.format('YYYY-MM-DD'));dateinput.siblings('.to').val(end.format('YYYY-MM-DD'));if(dateinput.siblings('.fromtime').length){dateinput.siblings('.fromtime').val(start.format('HH:mm'));dateinput.siblings('.totime').val(end.format('HH:mm'));}}};})(jQuery);popJSLibraryManager.register(popDateRange,['dateRange']);;(function($){popTypeahead={typeaheadSearchBtn:function(args){var t=this;var pageSection=args.pageSection,block=args.block,targets=args.targets;targets.click(function(){var control=$(this);var typeahead=control.closest('.pop-typeahead');t.executeTypeaheadSearch(pageSection,block,typeahead);});},typeaheadSearchInput:function(args){var t=this;var pageSection=args.pageSection,block=args.block,targets=args.targets;targets.keypress(function(e){if(event.which==13){event.preventDefault();var searchbox=$(this);var typeahead=searchbox.closest('.pop-typeahead');var input=typeahead.find('input[type="text"].tt-input');input.typeahead('close');t.executeTypeaheadSearch(pageSection,block,typeahead);}});},fillTypeahead:function(args){var t=this;var domain=args.domain,pageSection=args.pageSection,block=args.block,targets=args.targets;targets.each(function(){var typeahead=$(this);var urlparam=typeahead.data('urlparam');if(urlparam){var url=popManager.getBlockTopLevelURL(block);var values=getParam(urlparam,url);if(values){var database_key=typeahead.data('database-key');$.each(values,function(key,value){var datum=popManager.getItemObject(domain,database_key,value);t.selectTypeahead(domain,pageSection,block,typeahead,datum);});popBootstrap.execOpenParentCollapse(typeahead);}}});},selectDatum:function(args){var t=this;var domain=args.domain,pageSection=args.pageSection,block=args.block,targets=args.targets;targets.click(function(e){e.preventDefault();var link=$(this);var typeahead=$(link.data('target'));var value=link.data('objectid');var database_key=link.data('db-key');var datum=popManager.getItemObject(domain,database_key,value);t.selectTypeahead(domain,pageSection,block,typeahead,datum);});},selectableTypeahead:function(args){var t=this;var domain=args.domain,pageSection=args.pageSection,block=args.block,targets=args.targets;t.typeahead(domain,pageSection,block,targets);targets.each(function(){var typeahead=$(this);t.validateMaxSelected(pageSection,block,typeahead);typeahead.bind('typeahead:selected',function(obj,datum,name){var typeahead=$(this);t.selectTypeahead(domain,pageSection,block,typeahead,datum);});});},fetchlinkTypeahead:function(args){var t=this;var domain=args.domain,pageSection=args.pageSection,block=args.block,targets=args.targets;t.typeahead(domain,pageSection,block,targets);targets.each(function(){var typeahead=$(this);typeahead.bind('typeahead:selected',function(obj,datum,name){var selected=$(this);var typeahead=selected.closest('.pop-typeahead');popManager.click(datum.url,'',typeahead);});});},selectableTypeaheadTrigger:function(args){var t=this;var pageSection=args.pageSection,block=args.block,targets=args.targets;targets.on('close.bs.alert',function(){var alert=$(this);var typeahead=alert.closest('.pop-typeahead');var disable=t.getElementsToDisable(typeahead);disable.attr('disabled',false);disable.removeClass('disabled');var val=alert.children('input[type="hidden"]').val();typeahead.triggerHandler('close',[alert,val]);});},removeLocalStorageItem:function(args){var t=this;var path=args.path,key=args.key;args.result=args.result||key.startsWith('__'+path);},selectTypeahead:function(domain,pageSection,block,typeahead,datum){var t=this;var input=typeahead.find('input[type="text"].tt-input');t.triggerSelect(domain,pageSection,block,typeahead,datum);input.typeahead('val','');},executeTypeaheadSearch:function(pageSection,block,typeahead){var t=this;var input=typeahead.find('input[type="text"].tt-input');var url=popManager.getQueryUrl(pageSection,block);var val=input.typeahead('val');if(val){url=url.replace(M.JSPLACEHOLDER_QUERY,input.typeahead('val'));popManager.click(url,'',typeahead);}},getElementsToDisable:function(typeahead){var t=this;return typeahead.children('div.pop-operation').find('a, button, input');},typeahead:function(domain,pageSection,block,targets){var t=this;var pageSectionPage=popManager.getPageSectionPage(block);pageSectionPage.one('destroy',function(){t.destroy(targets);});block.one('rerender',function(action){t.destroy(targets);});targets.each(function(){var typeahead=$(this);var input=typeahead.find('input[type="text"]');var jsSettings=popManager.getJsSettings(domain,pageSection,block,typeahead);var dataset=[];$.each(jsSettings['dataset-components'],function(index,component){var datasetJsSettings=popManager.getJsSettings(domain,pageSection,block,component);dataset.push(datasetJsSettings['dataset']);});$.each(dataset,function(index,datasetunit){var layout=datasetunit['layout'];datasetunit.template=popManager.getScriptTemplate(layout);var args={};if(datasetunit.prefetch){args.url=datasetunit.prefetch;popJSLibraryManager.execute('typeaheadPrefetchURL',args);datasetunit.prefetch=args.url;}
if(datasetunit.remote){args.url=datasetunit.remote;popJSLibraryManager.execute('typeaheadRemoteURL',args);datasetunit.remote=args.url;}});var bloodhounds=[];var settings=[];var search;$.each(dataset,function(index,datasetunit){var limit=datasetunit.limit;if(dataset.length>1){if(limit>3){limit=3;}}
...
if(datasetunit.prefetch){options.prefetch={url:datasetunit.prefetch,thumbprint:datasetunit[M.KEYS_THUMBPRINT]};}
if(datasetunit.remote){options.remote={url:datasetunit.remote,wildcard:M.JSPLACEHOLDER_QUERY};}
var bloodhound=new Bloodhound(options);bloodhounds.push(bloodhound);var source;if(datasetunit.staticSuggestions){source=function(query,cb){var result=[];$.each(datasetunit.staticSuggestions,function(index,staticSuggestion){var suggestion={};$.each(staticSuggestion,function(key,value){if(typeof value=='string'){value=value.replace(M.JSPLACEHOLDER_QUERY,query);}
...
else if(settings.length==4){input.typeahead({highlight:true},settings[0],settings[1],settings[2],settings[3]);}});},triggerSelect:function(domain,pageSection,block,typeahead,datum){var t=this;if(typeahead.find('input[type="hidden"][value="'+datum.id+'"]').length==0){var jsSettings=popManager.getJsSettings(domain,pageSection,block,typeahead);var template=jsSettings['trigger-layout'];var pssId=popManager.getSettingsId(pageSection);var bsId=popManager.getSettingsId(block);var context=$.extend({itemObject:datum},jsSettings['datum-context']);var options={extendContext:context}
popJSRuntimeManager.setBlockURL(block);var html=popManager.getTemplateHtml(domain,pageSection,block,template,options);popManager.mergeHtml(html,typeahead.find('.pop-box'));popManager.runJSMethods(domain,pageSection,block,template,'last');popJSRuntimeManager.deleteBlockLastSessionIds(domain,pageSection,block,template);t.validateMaxSelected(pageSection,block,typeahead);typeahead.triggerHandler('selected',[datum]);}},validateMaxSelected:function(pageSection,block,typeahead){var t=this;if(typeahead.data('max-selected')){var numSelected=typeahead.find('input[type="hidden"]').length;if(numSelected==typeahead.data('max-selected')){var disable=t.getElementsToDisable(typeahead);disable.attr('disabled','disabled');disable.addClass('disabled');}}},removeSelected:function(typeahead,id){var t=this;var selected=typeahead.find('input[type="hidden"][value="'+id+'"]');var alert=selected.closest('.alert');alert.alert('close');},destroy:function(targets){var t=this;targets.find('input[type="text"].tt-input').typeahead('destroy');}};})(jQuery);popJSLibraryManager.register(popTypeahead,['selectableTypeahead','fillTypeahead','selectDatum','fetchlinkTypeahead','selectableTypeaheadTrigger','typeaheadSearchBtn','typeaheadSearchInput','removeLocalStorageItem']);;(function($){popGA={documentInitialized:function(){var t=this;var _gaq=_gaq||[];_gaq.push(['_setAccount',M.GOOGLEANALYTICS]);_gaq.push(['_trackPageview']);(function(){var ga=document.createElement('script');ga.type='text/javascript';ga.async=true;ga.src=('https:'==document.location.protocol?'https://ssl':'http://www')+'.google-analytics.com/ga.js';var s=document.getElementsByTagName('script')[0];s.parentNode.insertBefore(ga,s);})();},replaceState:function(args){var t=this;_gaq.push(['_trackPageview']);},};})(jQuery);popJSLibraryManager.register(popGA,['documentInitialized','replaceState']);;(function($){popPerfectScrollbar={scrollbarHorizontal:function(args){var t=this;var pageSection=args.pageSection,targets=args.targets;jQuery(document).ready(function($){targets.perfectScrollbar({suppressScrollY:true});});},scrollbarVertical:function(args){var t=this;var pageSection=args.pageSection,targets=args.targets;jQuery(document).ready(function($){targets.perfectScrollbar({suppressScrollX:true});});},getPosition:function(args){var t=this;var elem=args.elem;var scrollbar=elem.closest('.perfect-scrollbar').addBack('.perfect-scrollbar');if(scrollbar.length){var offsetReference=scrollbar.children('.perfect-scrollbar-offsetreference');if(offsetReference.length){return offsetReference.position().top*-1;}}
return 0;},doScroll:function(scrollbar,x,animate){var t=this;if(animate){scrollbar.animate({scrollTop:x});}
else{scrollbar.scrollTop(x);}
scrollbar.perfectScrollbar('update');},scrollToElem:function(args){var t=this;var elem=args.elem,position=args.position,animate=args.animate;var scrollbar=elem.closest('.perfect-scrollbar').addBack('.perfect-scrollbar');if(scrollbar.length){var x=0;if(position){var offsetReference=scrollbar.children('.perfect-scrollbar-offsetreference');if(!offsetReference.length){offsetReference=elem;}
x=position.offset().top-offsetReference.offset().top;}
t.doScroll(scrollbar,x,animate);return true;}
return false;},scrollTop:function(args){var t=this;var elem=args.elem,top=args.top,animate=args.animate;top=top||0;var scrollbar=elem.closest('.perfect-scrollbar').addBack('.perfect-scrollbar');if(scrollbar.length){t.doScroll(scrollbar,top,animate);return true;}
return false;},};})(jQuery);popJSLibraryManager.register(popPerfectScrollbar,['scrollbarHorizontal','scrollbarVertical','getPosition','scrollToElem','scrollTop']);;(function($){popFunctions={expandJSKeys:function(args){var t=this;var context=args.context;if(context[M.JS_FONTAWESOME]){context.fontawesome=context[M.JS_FONTAWESOME];}
if(context[M.JS_DESCRIPTION]){context.description=context[M.JS_DESCRIPTION];}},beep:function(args){beep();},showmore:function(args){var t=this;var targets=args.targets;targets.find(".pop-showmore-more").click(function(e){e.preventDefault();var link=$(this);var content=link.closest('.pop-content');content.find(".pop-showmore-more").addClass('hidden');content.find(".pop-showmore-less").removeClass('hidden');content.find(".pop-showmore-more-full").removeClass('hidden');});targets.find(".pop-showmore-less").click(function(e){e.preventDefault();var link=$(this);var content=link.closest('.pop-content');content.find(".pop-showmore-less").addClass('hidden');content.find(".pop-showmore-more").removeClass('hidden');content.find(".pop-showmore-more-full").addClass('hidden');});},hideEmpty:function(args){var t=this;var targets=args.targets;targets.closest('.pop-hide-empty').addClass('hidden');},imageResponsive:function(args){var t=this;var targets=args.targets;targets.find('div.pop-content').addBack('div.pop-content').find('img, .wp-caption').addClass('img-responsive');},printWindow:function(args){var t=this;var action=getParam(M.URLPARAM_ACTION,window.location.href);if(action==M.URLPARAM_ACTION_PRINT){jQuery(document).ready(function($){window.print();});}},copyHeader:function(args){var t=this;var targets=args.targets,link=args.relatedTarget;if(link){link=popManager.getOriginalLink(link);var header=link.data('header');if(header){targets.find('.pop-header').removeClass('hidden').find('.pop-box').html(header);}}},fillURLParamInput:function(args){var t=this;var block=args.block,targets=args.targets;targets.each(function(){var input=$(this);var val=t.getURLParamInputValue(block,input);input.val(val);});},fillAddonInput:function(args){var t=this;var targets=args.targets,link=args.relatedTarget;if(link){targets.each(function(){var input=$(this);t.fillInput(input,link);});}},fillModalInput:function(args){var t=this;var targets=args.targets;targets.each(function(){var input=$(this);var modal=input.closest('.modal');modal.on('show.bs.modal',function(e){var link=$(e.relatedTarget);t.fillInput(input,link);});});},fillAddonURLInput:function(args){var t=this;var domain=args.domain,targets=args.targets,link=args.relatedTarget;if(link){targets.each(function(){var input=$(this);t.fillURLInput(domain,input,link);});}},fillModalURLInput:function(args){var t=this;var domain=args.domain,targets=args.targets;targets.each(function(){var input=$(this);var modal=input.closest('.modal');modal.on('show.bs.modal',function(e){var link=$(e.relatedTarget);t.fillURLInput(domain,input,link);});});},switchTargetClass:function(args){var t=this;var targets=args.targets;targets.click(function(e){var button=$(this);if(button.is('a[href="#"]')){e.preventDefault();}
var target=$(button.data('target'))||$(document.body);var mode=button.data('mode')||'add';var classs=button.data('class');if(mode=='add'){target.addClass(classs);}
else if(mode=='remove'){target.removeClass(classs);}
else if(mode=='toggle'){target.toggleClass(classs);}});},doNothing:function(args){var t=this;var targets=args.targets;targets.click(function(e){e.preventDefault();});},fetchMore:function(args){var t=this;var pageSection=args.pageSection,block=args.block,targets=args.targets;block.on('beforeFetch',function(e,options){if(!options['skip-status']){targets.button('loading');}});block.on('fetchCompleted',function(e){if(popManager.stopFetchingBlock(pageSection,block)){targets.addClass('hidden');}
else{targets.removeClass('hidden');targets.button('reset');}});targets.click(function(e){e.preventDefault();popManager.fetchBlock(pageSection,block,{operation:M.URLPARAM_OPERATION_APPEND});});},fetchMoreDisable:function(args){var t=this;var block=args.block,targets=args.targets;block.on('beforeFetch',function(e,options){if(!options['skip-status']){targets.addClass('disabled').attr('disabled','disabled');}});block.on('fetchCompleted',function(e){targets.removeClass('disabled').attr('disabled',false);});},saveLastClicked:function(args){var t=this;var pageSection=args.pageSection,block=args.block,targets=args.targets;targets.click(function(e){var clicked=$(this);block.find('[data-action="retry"]').data('lastclicked','#'+clicked.attr('id')).removeClass('hidden');});},retrySendRequest:function(args){var t=this;var pageSection=args.pageSection,block=args.block,targets=args.targets;targets.click(function(e){e.preventDefault();var retryBtn=$(this);if(retryBtn.data('lastclicked')){$(retryBtn.data('lastclicked')).trigger('click');}})},highlight:function(args){var t=this;var targets=args.targets;targets.each(function(){var target=$(this);var pageSection=popManager.getPageSection(target);popManager.scrollToElem(pageSection,target,true);});},modalReloadEmbedPreview:function(args){var t=this;var domain=args.domain,pageSection=args.pageSection,targets=args.targets;targets.each(function(){var iframe=$(this);var block=popManager.getBlock(iframe);var jsSettings=popManager.getJsSettings(domain,pageSection,block,iframe);var urlType=jsSettings['url-type']||'embed';var modal=block.closest('.modal');modal.on('show.bs.modal',function(e){if(block.data('embed-iframe')){iframe=$(block.data('embed-iframe'));}
var link=$(e.relatedTarget);var url=t.getUrl(domain,link,true);if(urlType=='embed'){url=popManager.getEmbedUrl(url);}
else if(urlType=='api'){url=popManager.getAPIUrl(url);}
t.embedPreview(domain,pageSection,block,iframe,url);});});},reloadEmbedPreview:function(args){var t=this;var domain=args.domain,pageSection=args.pageSection,block=args.block,targets=args.targets;targets.each(function(){var connector=$(this);var input=$(connector.data('input-target'));input.change(function(){var input=$(this);t.embedPreviewFromInput(domain,pageSection,block,connector);});t.embedPreviewFromInput(domain,pageSection,block,connector);});},replaceCode:function(args){var t=this;var domain=args.domain,pageSection=args.pageSection,targets=args.targets;targets.each(function(){var input=$(this);var block=popManager.getBlock(input);var jsSettings=popManager.getJsSettings(domain,pageSection,block,input);var urlType=jsSettings['url-type']||'';var modal=input.closest('.modal');modal.on('show.bs.modal',function(e){var link=$(e.relatedTarget);var url=t.getUrl(domain,link);if(urlType=='embed'){url=popManager.getEmbedUrl(url);}
else if(urlType=='api'){url=popManager.getAPIUrl(url);}
t.execReplaceCode(input,url);});});},socialmedia:function(args){var t=this;var targets=args.targets;targets.one('mouseenter',function(){var socialmedia=$(this);socialmedia.children('a.socialmedia-item').each(function(){var item=$(this);var provider=item.data('provider');if(provider){var settings=M.SOCIALMEDIA[provider];if(settings){$.ajax({type:'GET',dataType:settings.dataType,url:settings['counter-url'].replace('%s',item.data('url')),success:function(json){if(!json.hasOwnProperty(settings.property))
return;var count=json[settings.property];if(count==0)return;item.children('.pop-counter').html(count);}});}}});});},mediaplayer:function(args){var t=this;var targets=args.targets;targets.mediaelementplayer();},reset:function(args){var t=this;var domain=args.domain,pageSection=args.pageSection,targets=args.targets;targets.click(function(e){e.preventDefault();var button=$(this);var block=popManager.getBlock(button);popManager.reset(domain,pageSection,block);popManager.closeMessageFeedback(block);});},browserUrl:function(args){var t=this;var targets=args.targets;targets.closest('form').on('beforeSubmit',function(){var url=window.location.href;targets.val(url);})},smallScreenHideCollapse:function(args){var t=this;var targets=args.targets;if($(window).width()<768){var targets=args.targets;targets.collapse('hide');}},cookies:function(args){var t=this;var targets=args.targets;targets.each(function(){var cookie=$(this);if(!($.cookie(cookie.data('cookieid')))){cookie.removeClass('hidden');}
cookie.children("button.close").click(function(e){e.preventDefault();var cookie=$(this).parent('.cookie');$.cookie(cookie.data('cookieid'),"set",{expires:90,path:"/"});});});},cookieToggleClass:function(args){var t=this;var pageSection=args.pageSection,targets=args.targets;targets.each(function(){var container=$(this);var cookieid=container.data('cookieid');t.cookieToggleClassInitial(pageSection,container);if(container.data('deletecookiebtn')){var selector=container.data('deletecookiebtn');var btn=(selector=='self'?container:$(selector));btn.click(function(e){$.cookie(cookieid,null,{expires:-1,path:"/"});});}
if(container.data('setcookiebtn')){var selector=container.data('setcookiebtn');var btn=(selector=='self'?container:$(selector));btn.click(function(e){$.cookie(cookieid,"set",{expires:180,path:"/"});});}
...
else{$.cookie(cookieid,"set",{expires:180,path:"/"});}});}});},sortable:function(args){var t=this;var pageSection=args.pageSection,block=args.block,targets=args.targets;targets.sortable({cursor:"move"});},onActionThenClick:function(args){var t=this;var targets=args.targets;targets.each(function(){var clickable=$(this);var trigger=$(clickable.data('triggertarget'));var action=clickable.data('trigger-action')||'click';if(clickable.data('triggertarget-internal')){trigger=trigger.find(clickable.data('triggertarget-internal'));}
trigger.on(action,function(e){clickable.trigger('click');});});},cookieToggleClassInitial:function(pageSection,container){var t=this;var cookieid=container.data('cookieid');var initial=container.data('initial')||'notset';if((initial=='set'&&$.cookie(cookieid))||(initial=='notset'&&!$.cookie(cookieid))){var target=$(container.data('cookietarget'));var collapse=container.data('cookiecollapse');if(collapse){jQuery(document).ready(function($){target.collapse(collapse);});}
var classs=container.data('cookieclass');if(classs){target.addClass(classs);}}},embedPreviewFromInput:function(domain,pageSection,block,connector){var t=this;var input=$(connector.data('input-target'));var url=input.val();if(url&&isUrlValid(url)){var iframe=$(connector.data('iframe-target'));var merged=t.embedPreview(domain,pageSection,block,iframe,url);connector.data('iframe-target','#'+merged.targetContainer.attr('id'));}},getURLParamInputValue:function(block,input){var t=this;var val,urlparam=input.data('urlparam');if(urlparam){var url=popManager.getBlockTopLevelURL(block);val=getParam(urlparam,url);}
return val||'';},embedPreview:function(domain,pageSection,block,iframe,url){var t=this;var options={operation:M.URLPARAM_OPERATION_REPLACEINLINE,extendContext:{src:url},'merge-target':'#'+iframe.attr('id')}
var template=iframe.data('templateid');popJSRuntimeManager.setBlockURL(block);var merged=popManager.mergeTargetTemplate(domain,pageSection,block,template,options);popManager.runJSMethods(domain,pageSection,block,template,'full');var iframeid=popJSRuntimeManager.getLastGeneratedId(domain,popManager.getSettingsId(pageSection),popManager.getSettingsId(block),template);block.data('embed-iframe','#'+iframeid);return merged;},fillInput:function(input,link){var t=this;link=popManager.getOriginalLink(link);var val,attr=input.data('attr');if(attr){val=link.data(attr);}
val=val||'';input.val(val);},fillURLInput:function(domain,input,link){var t=this;link=popManager.getOriginalLink(link);var url=t.getUrl(domain,link);input.val(url);},getUrl:function(domain,link,use_pageurl){var t=this;link=popManager.getOriginalLink(link);var url=link.data('target-url');if(!url){var blockTarget=$(link.data('blocktarget'));var pageSection=popManager.getPageSection(blockTarget);url=popManager.getBlockFilteringUrl(domain,pageSection,blockTarget,use_pageurl);}
return url;},execReplaceCode:function(input,url){var t=this;var placeholder=input.data('code-placeholder');var code=placeholder.format(url);input.val(code);},};})(jQuery);popJSLibraryManager.register(popFunctions,['expandJSKeys','showmore','hideEmpty','imageResponsive','printWindow','copyHeader','fillURLParamInput','fillAddonInput','fillModalInput','fillAddonURLInput','fillModalURLInput','switchTargetClass','doNothing','fetchMore','fetchMoreDisable','saveLastClicked','retrySendRequest','highlight','replaceCode','modalReloadEmbedPreview','reloadEmbedPreview','socialmedia','mediaplayer','reset','browserUrl','smallScreenHideCollapse','cookies','cookieToggleClass','sortable','onActionThenClick','beep']);;(function($){popBlockFunctions={hideIfEmptyBlock:function(args){var t=this;var domain=args.domain,pageSection=args.pageSection,block=args.block;if(popManager.hideIfEmpty(domain,pageSection,block)){block.addClass('hidden');}
else{block.removeClass('hidden');}}};})(jQuery);popJSLibraryManager.register(popBlockFunctions,['hideIfEmptyBlock']);;(function($){popBootstrapCarousel={carouselStatic:function(args){var t=this;var pageSection=args.pageSection,block=args.block,targets=args.targets;t.prepareCarousel(pageSection,block,targets);targets.carousel({interval:false,wrap:false});},carouselAutomatic:function(args){var t=this;var pageSection=args.pageSection,block=args.block,targets=args.targets;block.on('beforeReload',function(e){targets.carousel('pause');});t.prepareCarousel(pageSection,block,targets);if(carousel.find('.item.active').length){if(gdBootstrapCarousel.numberSlides(carousel)>=2){if($(window).width()>=768){carousel.carousel();}
else{carousel.carousel({interval:false,wrap:false});}
carousel.find('.pop-carousel-controls').removeClass('hidden');}
else{carousel.find('.pop-carousel-controls').addClass('hidden');}}},prepareCarousel:function(pageSection,block,targets){var t=this;block.on('rendered',function(){t.targetsAddActive(targets);});t.targetsAddActive(targets);},targetsAddActive:function(targets){var t=this;targets.each(function(){var carousel=$(this);t.addActive(carousel);});},addActive:function(carousel){var t=this;if(carousel.find('.item.active').length==0){carousel.find('.item:first-child').addClass('active');}},showElement:function(carousel,elem){var t=this;var slide=elem.closest('.item');var slideNumber=slide.parent().children().index(slide);t.slideTo(carousel,slideNumber);},slideTo:function(carousel,number){var t=this;carousel.carousel(number);},next:function(carousel){var t=this;carousel.carousel('next');},prev:function(carousel){var t=this;carousel.carousel('prev');},isFirstSlide:function(carousel){var t=this;return carousel.find('.item:first-child').hasClass('active');},isLastSlide:function(carousel){var t=this;return carousel.find('.item:last-child').hasClass('active');},isNthSlide:function(carousel,position){var t=this;return carousel.find('.item:nth-child('+position+')').hasClass('active');},isNthLastSlide:function(carousel,position){var t=this;return carousel.find('.item:nth-last-child('+position+')').hasClass('active');},numberSlides:function(carousel){var t=this;return carousel.find('.item').length;},};})(jQuery);(function($){popBootstrapCarouselControls={initBlockRuntimeMemory:function(args){var t=this;var pageSection=args.pageSection,block=args.block,mempage=args.runtimeMempage;mempage.carouselControls={};t.resetBlockRuntimeMemory(pageSection,block);},carouselControls:function(args){var t=this;var pageSection=args.pageSection,block=args.block,targets=args.targets;var carousel=t.getCarousel(targets);var controlNext=targets.children('.pop-carousel-control.right');t.controlCarouselNext(pageSection,block,carousel,targets,controlNext);var controlPrev=targets.children('.pop-carousel-control.left');t.controlCarouselPrev(pageSection,block,carousel,targets,controlPrev);block.on('fetchDomainCompleted',function(e,status,domain){t.handleReload(domain,pageSection,block,targets);});t.setEnabledDisabled(pageSection,block,targets);},getRuntimeMemoryPage:function(pageSection,targetOrId){var t=this;return popManager.getRuntimeMemoryPage(pageSection,targetOrId).carouselControls;},resetBlockRuntimeMemory:function(pageSection,targetOrId){var t=this;var mempage=t.getRuntimeMemoryPage(pageSection,targetOrId);var empty={state:'',};$.extend(mempage,empty);},slideNext:function(pageSection,block,carousel,controlNext){var t=this;if(popBootstrapCarousel.isNthLastSlide(carousel,2)&&popManager.stopFetchingBlock(pageSection,block)){t.disable(controlNext);}
t.enable(controlNext.siblings('.pop-carousel-control.left'));popBootstrapCarousel.next(carousel);},controlCarouselNext:function(pageSection,block,carousel,carouselControls,control){var t=this;var blockQueryState=popManager.getBlockQueryState(pageSection,block);control.click(function(e){e.preventDefault();var control=$(this);if(t.disabled(control))return;if(popBootstrapCarousel.isLastSlide(carousel)){if(blockQueryState.loading.length||popManager.stopFetchingBlock(pageSection,block)){return;}
...
if(popBootstrapCarousel.isLastSlide(carousel)&&popManager.stopFetchingBlock(pageSection,block)){t.disable(controlNext);}
else{t.enable(controlNext);}}},disable:function(control){var t=this;control.addClass('disabled');control.attr('disabled',true);},enable:function(control){var t=this;control.removeClass('disabled');control.attr('disabled',false);},disabled:function(control){var t=this;return control.hasClass('disabled');}};})(jQuery);popJSLibraryManager.register(popBootstrapCarousel,['carouselStatic','carouselAutomatic']);popJSLibraryManager.register(popBootstrapCarouselControls,['initBlockRuntimeMemory','controlCarouselPrev','controlCarouselNext','carouselControls']);;(function($){popModals={modalForm:function(args){var t=this;var pageSection=args.pageSection,targets=args.targets;targets.on('show.bs.modal',function(e){var modal=$(this);var link=$(e.relatedTarget);var blocks=$(modal.data('initjs-targets'));blocks.each(function(){var block=$(this);popManager.closeMessageFeedback(block);})});}};})(jQuery);popJSLibraryManager.register(popModals,['modalForm']);;(function($){popControls={controlPrint:function(args){var t=this;var domain=args.domain,pageSection=args.pageSection,block=args.block,targets=args.targets;targets.click(function(e){e.preventDefault();var control=$(this);var url=t.getBlockFilteringUrl(domain,pageSection,control,true);url=popManager.getPrintUrl(url);popSystem.openPrint(url);});},controlSocialMedia:function(args){var t=this;var domain=args.domain,pageSection=args.pageSection,block=args.block,targets=args.targets;targets.click(function(e){e.preventDefault();var control=$(this);var url=control.data('shareurl')?control.data('shareurl'):t.getBlockFilteringUrl(domain,pageSection,control,true);var title=control.data('sharetitle')?control.data('sharetitle'):popManager.documentTitle;var provider=control.data('provider');var settings=M.SOCIALMEDIA[provider];var shareUrl=settings['share-url'].replace(new RegExp('%url%','g'),encodeURIComponent(url)).replace(new RegExp('%title%','g'),encodeURIComponent(title));popSystem.openSocialMedia(shareUrl);});},getBlockFilteringUrl:function(domain,pageSection,control,use_pageurl){var t=this;var targetBlock=$(control.data('blocktarget'));return popManager.getBlockFilteringUrl(domain,pageSection,targetBlock,use_pageurl);},};})(jQuery);popJSLibraryManager.register(popControls,['controlPrint','controlSocialMedia']);
;/*
