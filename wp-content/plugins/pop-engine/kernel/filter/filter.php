...
class GD_Filter extends GD_FilterBase {
	...
	function get_metaquery() {
		...
		foreach ($filtercomponents as $filtercomponent) {
			...
			$meta_query = array_merge($meta_query, $filtercomponent->get_metaquery());
		}
	...
	function get_author() {
		...
		foreach ($filtercomponents as $filtercomponent) {
			...
			$author = array_merge($author, $filtercomponent->get_author());
		}
	...
	function get_poststatus() {
		...
		foreach ($filtercomponents as $filtercomponent) {
			...
			$status = array_merge($status, $filtercomponent->get_poststatus());
		}
	...
	function get_categories() {
		...
		foreach ($filtercomponents as $filtercomponent) {
			...
			$categories = array_merge($categories, $filtercomponent->get_categories());
		}
	...
	function get_taxonomies() {
		...
		foreach ($filtercomponents as $filtercomponent) {
			...
			$taxonomies = array_merge_recursive($taxonomies, $filtercomponent->get_taxonomies());
		}
	...
	function get_tags() {
		...
		foreach ($filtercomponents as $filtercomponent) {
			...
			$tags = array_merge($tags, $filtercomponent->get_tags());
		}
	...
	function get_order() {
		...
		foreach ($filtercomponents as $filtercomponent) {
			...
			if ($order = $filtercomponent->get_order()) {
	...
	function get_search() {
		...
		foreach ($filtercomponents as $filtercomponent) {
			...
			if ($filtercomponent->get_search()) {
				$search = $filtercomponent->get_search();
	...
	function get_postdates() {
		...
		foreach ($filtercomponents as $filtercomponent) {
			...
			if ($filtercomponent->get_postdates()) {
				$postdates = $filtercomponent->get_postdates();
