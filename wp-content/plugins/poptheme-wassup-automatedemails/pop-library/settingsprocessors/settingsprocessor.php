...
class PoPTheme_Wassup_AutomatedEmails_Template_SettingsProcessor extends GD_Template_SettingsProcessorBase {
	...
	function get_checkpoints($hierarchy) {
		...
		if ($hierarchy == GD_SETTINGS_HIERARCHY_PAGE) {
			...
			return array(
				POPTHEME_WASSUP_AUTOMATEDEMAILS_PAGE_LATESTPOSTS_WEEKLY => PoPTheme_Wassup_PoPSystem_SettingsProcessor_CheckpointUtils::get_checkpoint($hierarchy, WASSUP_CHECKPOINT_SYSTEMACCESSVALID),
				POPTHEME_WASSUP_AUTOMATEDEMAILS_PAGE_SINGLEPOST_SPECIAL => PoPTheme_Wassup_PoPSystem_SettingsProcessor_CheckpointUtils::get_checkpoint($hierarchy, WASSUP_CHECKPOINT_SYSTEMACCESSVALID),
			);
		}
