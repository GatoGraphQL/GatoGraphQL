...
class GD_Custom_DataLoad_FieldProcessor_Posts extends GD_DataLoad_FieldProcessor_Posts {
	...
	function get_value($resultitem, $field) {
		...
		// (So that won't have both project-caterories and discussion-categories)
		...
		switch ($field) {
			...
			case 'content' :
				...
				if (
					(POPTHEME_WASSUP_SECTIONPROCESSORS_CAT_PROJECTS && POPTHEME_WASSUP_SECTIONPROCESSORS_CAT_PROJECTLINKS && ($cat == POPTHEME_WASSUP_SECTIONPROCESSORS_CAT_PROJECTS) && has_category(POPTHEME_WASSUP_SECTIONPROCESSORS_CAT_PROJECTLINKS, $post)) ||
			...
			case 'projectcategories':
				if (POPTHEME_WASSUP_SECTIONPROCESSORS_CAT_PROJECTS && ($cat == POPTHEME_WASSUP_SECTIONPROCESSORS_CAT_PROJECTS)) {
					
					return GD_MetaManager::get_post_meta($this->get_id($post), GD_METAKEY_POST_PROJECTCATEGORIES);
				}
				break;

			case 'projectcategories-strings':				
				
				if (POPTHEME_WASSUP_SECTIONPROCESSORS_CAT_PROJECTS && ($cat == POPTHEME_WASSUP_SECTIONPROCESSORS_CAT_PROJECTS)) {
					$selected = $this->get_value($post, 'projectcategories');
					$params = array(
						'selected' => $selected
					);
					$projectcategories = new GD_FormInput_ProjectCategories($params);
					return $projectcategories->get_selected_value();
				}
				break;

			case 'has-projectcategories':				
				
				if (POPTHEME_WASSUP_SECTIONPROCESSORS_CAT_PROJECTS && ($cat == POPTHEME_WASSUP_SECTIONPROCESSORS_CAT_PROJECTS)) {
					
					if ($this->get_value($post, 'projectcategories')) {
						return true;
					}
					return false;
				}
