...
window.popMap = {
	...
	getMarkersInfo : function(pageSection, block, map) {
		...
		var mempage = popMapMemory.getEmtpyBlockRuntimeMemory();
		try {
			mempage = popMapMemory.getRuntimeMemoryPage(markersPageSection, markersBlock);
		}
		catch(err) {
			// Do nothing
...
// Initialize
//-------------------------------------------------
popJSLibraryManager.register(popMap, ['map', 'mapModal', 'mapStandalone']);
