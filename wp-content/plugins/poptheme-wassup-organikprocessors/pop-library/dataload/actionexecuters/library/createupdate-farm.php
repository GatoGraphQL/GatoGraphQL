...
class GD_CreateUpdate_Farm extends GD_CustomCreateUpdate_Post {

	protected function get_categories($form_data) {

		return array(POPTHEME_WASSUP_ORGANIKPROCESSORS_CAT_FARMS);
	}
	...
	protected function get_form_data($atts) {
		...
		$form_data = array_merge(
			$form_data,
			array(
				// 'farmcategories' => $gd_template_processor_manager->get_processor(GD_TEMPLATE_FORMCOMPONENT_FARMCATEGORIES)->get_value(GD_TEMPLATE_FORMCOMPONENT_FARMCATEGORIES, $atts),
	...
	protected function additionals($post_id, $form_data) {
		...
		// Action Categories
		// GD_MetaManager::update_post_meta($post_id, GD_METAKEY_POST_FARMCATEGORIES, $form_data['farmcategories']);
...
// $gd_createupdate_farm = new GD_CreateUpdate_Farm();