...
class UploadHandlerS3 {
    ...
    function __construct($vars, $options = null, $initialize = true, $error_messages = null) {
        ...
        $this->options = array(
            ...
            'upload_dir' =>'s3://'.$this->bucket.'/'.$this->originalprefix,
            'server_url'=> $this->get_full_url().'/'.$this->originalprefix, //$user->publish->path, 
            'upload_url'=> "https://" . $this->bucket . ".s3.amazonaws.com/".$this->originalprefix, 
            'user_dirs' => false,
            'mkdir_mode' => 0755,
            'param_name' => 'files',
            ...
            */
            ...
            'image_versions' => array(
                ...
                'photo' => array(
                    'upload_dir' =>'s3://'.$this->bucket.'/'.$this->photoprefix,
                    'upload_url'=> "https://" . $this->bucket . ".s3.amazonaws.com/".$this->photoprefix, 
                    'max_width' => $this->vars['photo_maxwidth'],
                    'max_height' => $this->vars['photo_maxheight'],
                    'crop' => false
                ),
                'thumb' => array(
                    'upload_dir' =>'s3://'.$this->bucket.'/'.$this->thumbprefix,
                    'upload_url'=> "https://" . $this->bucket . ".s3.amazonaws.com/".$this->thumbprefix, 
        ...
        foreach ($thumb_sizes as $thumb_size) {
            $this->options['image_versions']['thumb-'.$thumb_size] = array(
                'upload_dir' =>'s3://'.$this->bucket.'/'.$this->avatarsprefix.$thumb_size.'/',
                'upload_url'=> "https://" . $this->bucket . ".s3.amazonaws.com/".$this->avatarsprefix.$thumb_size.'/', 
    ...
    protected function get_full_url() {
        ...
        $https = (!empty($_SERVER['HTTPS']) && strcasecmp($_SERVER['HTTPS'], 'on') === 0) || (strpos($_SERVER['HTTP_X_FORWARDED_PROTO'], 'https') !== false);
        return
            ($https ? 'https://' : 'http://').
    ...
    protected function set_additional_file_properties($file) {
        ...
        list($file_path, $photo_path) =
            $this->get_scaled_image_file_paths($file->name, 'photo');
    ...
    protected function gd_create_scaled_image($file_name, $version, $options) {
        ...
        if ($scale > 1) {
            ...
            if ($file_path !== $new_file_path) {
                $ret = copy($file_path, $new_file_path);
            }
            if ($ret) {

                // Re-create the key by substracting the s3 domain from the new_file_path
                $key = substr($new_file_path, strlen('s3://'.$this->bucket.'/'));
        ...
        $this->gd_set_image_object($file_path, $new_img);
         ...
         $context = stream_context_create(array(
            's3' => array(
                ...
                'ContentType' => $this->get_file_type($file_path)
            )
        ));
