...
else if(modals.indexOf(psId)>-1){popCustomBootstrap.activateModals(pageSection,newDOMs);}},isUserIdSameAsLoggedInUser:function(args){var t=this;var domain=args.domain;if(popUserAccount.isLoggedIn(domain)){var user_id=args.input;return(popUserAccount.getLoggedInUserId(domain)==user_id);}
return false;},getEmbedUrl:function(args){var t=this;var url=args.url;return t.addURLParams(add_query_arg(M.URLPARAM_THEMEMODE,M.THEMEMODE_WASSUP_EMBED,url));},getUnembedUrl:function(args){var t=this;var url=args.url;return removeQueryFields(url,[M.URLPARAM_THEMEMODE]);},getPrintUrl:function(args){var t=this;var url=args.url;return t.addURLParams(add_query_arg(M.URLPARAM_ACTION,M.URLPARAM_ACTION_PRINT,add_query_arg(M.URLPARAM_THEMEMODE,M.THEMEMODE_WASSUP_PRINT,url)));},scrollToElem:function(args){var t=this;var elem=args.elem,position=args.position,animate=args.animate;var pageSection=popManager.getPageSection(position);var top=position.offset().top-pageSection.offset().top;return t.execScrollTop(elem,top,animate);},scrollTop:function(args){var t=this;var elem=args.elem,top=args.top,animate=args.animate;return t.execScrollTop(elem,top,animate);},getPosition:function(args){var t=this;var elem=args.elem;if(elem.attr('id')==M.PS_MAIN_ID&&!elem.hasClass('perfect-scrollbar')){return body.scrollTop;}
return 0;},execScrollTop:function(elem,top,animate){var t=this;if(elem.attr('id')==M.PS_MAIN_ID&&!elem.hasClass('perfect-scrollbar')){if(animate){$(document.body).animate({scrollTop:top},'fast');}
else{window.scrollTo(0,top);}
return true;}
return false;},addURLParams:function(url){var t=this;if(M.THEMESTYLE){url=add_query_arg(M.URLPARAM_THEMESTYLE,M.THEMESTYLE,url);}
return url;},handleWindow:function(pageSection,exitSize){var t=this;var windoww=pageSection.closest('.window');if(popWindow.getSize(windoww)==exitSize){popWindow.maximize(windoww);}},};})(jQuery);popJSLibraryManager.register(popCustomFunctions,['addPageSectionIds','pageSectionNewDOMsBeforeInitialize'],true);popJSLibraryManager.register(popCustomFunctions,['pageSectionInitialized','getEmbedUrl','getUnembedUrl','getPrintUrl','isUserIdSameAsLoggedInUser','getPosition','scrollToElem','scrollTop']);;(function($){popUserRole={filterByCommunity:function(args){var t=this;var domain=args.domain,pageSection=args.pageSection,block=args.block,targets=args.targets;targets.change(function(e){var checkbox=$(this);var user_id=checkbox.val();var communityTypeahead=$(checkbox.data('target'));if(checkbox.is(':checked')){var user=checkbox.data('user-datum');popTypeahead.triggerSelect(domain,pageSection,block,communityTypeahead,user);}
