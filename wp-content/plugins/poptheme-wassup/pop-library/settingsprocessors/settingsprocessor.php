	...
	function silent_document($hierarchy) {
		...
		if ($hierarchy == GD_SETTINGS_HIERARCHY_PAGE) {
			...
			return array(
				...
				POP_COREPROCESSORS_PAGE_LOADERS_INITIALIZEDOMAIN => true,
	...
	function get_checkpoints($hierarchy) {
		...
		if ($hierarchy == GD_SETTINGS_HIERARCHY_PAGE) {
			...
			return array(
				...
				POP_COREPROCESSORS_PAGE_LOADERS_INITIALIZEDOMAIN => Wassup_SettingsProcessor_CheckpointUtils::get_checkpoint($hierarchy, WASSUP_CHECKPOINT_DOMAINVALID),//$profile_datafromserver,
	...
	function get_page_blockgroups($hierarchy, $include_common = true) {
		...
		if ($include_common) {
			...
			$pageblockgroups_allothers = array(
				...
				POP_COREPROCESSORS_PAGE_LOADERS_INITIALIZEDOMAIN => GD_TEMPLATE_BLOCKGROUP_INITIALIZEDOMAIN,
	...
	function get_page_blocks($hierarchy, $include_common = true) {
		...
		if ($include_common) {
			...
			$pageblocks_allothers = array(
				...
				// Profile
				POP_COREPROCESSORS_PAGE_LOADERS_INITIALIZEDOMAIN => null,
		...
		if ($hierarchy == GD_SETTINGS_HIERARCHY_HOME || $hierarchy == GD_SETTINGS_HIERARCHY_PAGE) {
			...
			elseif ($hierarchy == GD_SETTINGS_HIERARCHY_PAGE) {
				...
				$pageblocks_common = array(
					POP_FRONTENDENGINE_PAGE_EXTERNAL => GD_TEMPLATE_BLOCK_EXTERNAL,
				);
