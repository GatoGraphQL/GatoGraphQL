...
if (class_exists("EM_Gateway")) { 
	...
	class EM_Gateway_Online extends EM_Gateway {
		...
		var $title;
		var $status = 4;
		var $status_txt = 'Awaiting Online Payment';		// Change Leo
		...
		function __construct() {
			...
			parent::__construct();
			$this->status_txt = sprintf(__('Awaiting %s Payment', 'poptheme-wassup'), __($this->title, 'poptheme-wassup'));			// Change Leo
		...
		function booking_form_feedback( $return, $EM_Booking = false ){
			...
			if( is_object($EM_Booking) && $this->uses_gateway($EM_Booking) ){
				if( !empty($return['result']) && $EM_Booking->get_price() > 0 && $EM_Booking->booking_status == $this->status ){
					...
					$gateway_url = $this->get_gateway_url();				// Change Leo
					$gateway_vars = $this->get_gateway_vars($EM_Booking);					// Change Leo			
		...
		function booking_form_feedback_fallback( $feedback ){
			...
			if( is_object($EM_Booking) ){
				$feedback .= "<br />" . sprintf(__('To finalize your booking, please click the following button to proceed to %s.','em-pro'), $this->title) . $this->em_my_bookings_booking_actions('',$EM_Booking);			// Change Leo
			}
			return $feedback;
		}
		...
		function em_my_bookings_booking_actions( $message, $EM_Booking){
			...
			if($this->uses_gateway($EM_Booking) && $EM_Booking->booking_status == $this->status){
				...
				if( $pending_payments == 0 ){
					//user owes money!
					$gateway_vars = $this->get_gateway_vars($EM_Booking);			// Change Leo
					$form = '<form action="'.$this->get_gateway_url().'" method="post">';			// Change Leo
		 ...
		 */
		function em_gateway_js(){			// Change Leo
		 ...
		 */
		function mysettings() {			// Change Leo
			global $EM_options;
			?>
			...
			<tbody>
			  ...
			  <tr valign="top">
				  ...
				  <td>
					<input type="text" name="<?php echo $this->gateway ?>_return" value="<?php esc_attr_e(get_option('em_'. $this->gateway . "_return" )); ?>" style='width: 40em;' /><br />
			  ...
			  <tr valign="top">
				  ...
				  <td>
					<input type="text" name="<?php echo $this->gateway ?>_cancel_return" value="<?php esc_attr_e(get_option('em_'. $this->gateway . "_cancel_return" )); ?>" style='width: 40em;' /><br />
	 ...
	 */
	function em_gateway_booking_timeout($args){
		global $wpdb;
		$gateway = $args['gateway'];
		
		//Get a time from when to delete
		$minutes_to_subtract = absint(get_option('em_'.$gateway.'_booking_timeout'));
