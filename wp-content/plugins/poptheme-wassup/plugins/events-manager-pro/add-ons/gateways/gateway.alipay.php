...
if (class_exists("EM_Gateway_Online")) {
	class EM_Gateway_Alipay extends EM_Gateway_Online {
		 ...
		 */
		function get_gateway_vars($EM_Booking){
			...
			$payment_method = get_option('em_'.$this->gateway.'_payment_method');
			$services = array(	
			...
			*/
			...
			$alipay_vars = array(
				...
				"partner"			=> trim(get_option('em_'. $this->gateway . "_partner_id" )),
				"seller_email"		=> trim(get_option('em_'. $this->gateway . "_alipay_account" )),
			...
			$mysign = $this->build_sign($alipay_vars);		
			$alipay_vars['sign'] = $mysign;
			$alipay_vars['sign_type'] = 'MD5';
	
			return apply_filters('em_gateway_alipay_get_paypal_vars', $alipay_vars, $EM_Booking, $this);
		}
		
		function build_sign($vars) {
			...
			$key = trim(get_option('em_'. $this->gateway . "_security_key" ));
		...
		function handle_payment_return() {
			...
			if ( isset($_POST['seller_id']) && trim($_POST['seller_id']) == trim(get_option('em_'. $this->gateway . "_partner_id" )) ){
				...
				$mysign = $this->build_sign($_POST);
				
				$responseTxt = '';
				if (!empty($_POST["notify_id"])) {
					
					$veryfy_url = 'http://notify.alipay.com/trade/notify_query.do?';
					$veryfy_url .= "partner=" . get_option('em_'. $this->gateway . "_partner_id" );
				...
				if( !empty($EM_Booking->booking_id) && count($custom_values) == 2 ){
					...
					// process Alipay response
					 if($_POST['trade_status'] == 'TRADE_FINISHED' || $_POST['trade_status'] == 'TRADE_SUCCESS') {
						...
						if( $amount >= $EM_Booking->get_price(false, false, true) && (!get_option('em_'.$this->gateway.'_manual_approval', false) || !get_option('dbem_bookings_approval')) ){
					...
					if($_POST['trade_status'] == 'TRADE_FINISHED' || $_POST['trade_status'] == 'TRADE_SUCCESS') {
						$message = apply_filters('em_gateway_alipay_bad_booking_email',"
	...
	Events Manager
						...
						}else{ $event_details = __('Unknown','poptheme-wassup'); }
		...
		function mysettings() {
			...
			<h3><?php echo sprintf(__('%s Options','poptheme-wassup'), $this->title); ?></h3>
			<p><?php echo __('<strong>Important:</strong>In order to connect Alipay with your site, you need to configure the return URL.'); echo " ". sprintf(__('Your return url is %s','poptheme-wassup'),'<code>'.$this->get_payment_return_url().'</code>'); ?></p> 
			<p><?php echo sprintf(__('Please visit the <a href="%s">documentation</a> for further instructions.','poptheme-wassup'), 'http://wp-events-plugin.com/documentation/'); ?></p>
			<table class="form-table">
			<tbody>
			  <tr valign="top">
				  <th scope="row"><?php _e('Alipay account', 'poptheme-wassup') ?></th>
					  <td><input type="text" name="<?php echo $this->gateway ?>_account" value="<?php esc_attr_e( get_option('em_'. $this->gateway . "_account" )); ?>" style="width: 40em;"/>
					  <br />
			  ...
			  <tr valign="top">
				  <th scope="row"><?php _e('Alipay partner ID', 'poptheme-wassup') ?></th>
					  <td><input type="text" name="<?php echo $this->gateway ?>_partner_id" value="<?php esc_attr_e( get_option('em_'. $this->gateway . "_partner_id" )); ?>" style="width: 40em;"/>
					  <br />
			  ...
			  <tr valign="top">
				  <th scope="row"><?php _e('Alipay security key', 'poptheme-wassup') ?></th>
					  <td><input type="text" name="<?php echo $this->gateway ?>_security_key" value="<?php esc_attr_e( get_option('em_'. $this->gateway . "_security_key" )); ?>" style="width: 40em;"/>
					  <br />
				  ...
				  <td>
					<select name="<?php echo $this->gateway ?>_payment_method">
						...
						foreach($options as $key => $value){
							...
							echo '<option value="'.$key.'"'.$selected.'>'.__($value,'poptheme-wassup').'</option>';
						}
						?>
					  </select>
					  <br />
	...
	//add_action('emp_alipay_cron', 'em_gateway_booking_timeout');
}
?>