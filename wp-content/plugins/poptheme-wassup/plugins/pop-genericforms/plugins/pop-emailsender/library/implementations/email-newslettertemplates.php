...
class PoP_EmailSender_Templates_Newsletter extends PoP_EmailSender_Templates_Simple {
	...
	function get_emailframe_beforefooter(/*$frame, */$title, $emails, $names, $template) {
		...
		$email = $emails[0];
		$verificationcode = PoP_GenericForms_NewsletterUtils::get_email_verificationcode($email);
		$url = get_permalink(POP_GENERICFORMS_PAGE_NEWSLETTERUNSUBSCRIPTION);
		$url = add_query_arg(GD_TEMPLATE_FORMCOMPONENT_NEWSLETTEREMAIL, $email, $url);
		$url = add_query_arg(GD_TEMPLATE_FORMCOMPONENT_NEWSLETTEREMAILVERIFICATIONCODE, $verificationcode, $url);
