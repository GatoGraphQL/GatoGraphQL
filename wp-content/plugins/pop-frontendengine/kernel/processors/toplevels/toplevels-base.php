...
class PoPFrontend_Template_Processor_TopLevelsBase extends GD_Template_ProcessorBase {
		...
		if (empty($ret)) {
			...
			foreach ($this->get_modules($template_id) as $module) {
				...
				$ret = array_unique(array_merge(
					$ret,
					$gd_template_processor_manager->get_processor($module)->get_dynamic_templates_sources($module, $module_atts)
				));
			}
		}
	...
	function get_templates_extra_sources($template_id, $atts) {
		...
		foreach ($this->get_modules($template_id) as $module) {
			...
			$ret = array_merge(
				$ret,
				$gd_template_processor_manager->get_processor($module)->get_templates_extra_sources($module, $module_atts)
			);
		}
