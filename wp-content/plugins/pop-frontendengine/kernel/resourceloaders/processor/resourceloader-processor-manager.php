...
class PoP_ResourceLoaderProcessor_Manager {
	...
	function get_subtype($resource) {
		...
		return $this->get_processor($resource)->get_subtype($resource);
	}
	...
	function get_handle($resource) {
		...
		return $this->get_processor($resource)->get_handle($resource);
	}
	...
	function filter_vendor($resources) {
		...
		return array_values(array_filter($resources, array($this, 'is_vendor')));
	}

	function is_vendor($resource) {

		return $this->get_processor($resource)->get_subtype($resource) == POP_RESOURCELOADER_RESOURCESUBTYPE_VENDOR;
	}

	function filter_dynamic($resources) {

		return array_values(array_filter($resources, array($this, 'is_dynamic')));
	}

	function is_dynamic($resource) {

		return $this->get_processor($resource)->get_subtype($resource) == POP_RESOURCELOADER_RESOURCESUBTYPE_DYNAMIC;
	}

	function filter_template($resources) {

		return array_values(array_filter($resources, array($this, 'is_template')));
	}

	function is_template($resource) {

		return $this->get_processor($resource)->get_subtype($resource) == POP_RESOURCELOADER_RESOURCESUBTYPE_TEMPLATE;
	}
