...
class PoP_Frontend_ResourceLoaderMappingParser {
		...
		for ($i = 0; $i < count($tokens); ++$i) {
			...
			if($this->_j_token_is_word($token)) {
				...
				if ($s == "{") {
					...
					// Are we declaring the object? Check if it has the shape of: "popManager = {"
			...
			elseif ($this->isLiteral($token) && ($recordPublicFunctions || $recordMethodExecution)) {
				...
				if ($t == 2) {
					...
					if ($currentObject && $lastFunction && $sp == '.' && $sn == '(' && in_array($lastSymbol, $jsObjects)/* && !in_array($lastSymbol, $ignoreObjects)*/) {
						...
						if ($lastSymbol == 'popJSLibraryManager') {
							...
							elseif ($maybeFunctionCall == 'execute') {
								...
								// Keep track of the popJSLibraryManager.execute calls inside the current function
 ...
 * Initialization
 * ---------------------------------------------------------------------------------------------------------------*/
global $pop_frontend_resourceloader_mappingparser;
$pop_frontend_resourceloader_mappingparser = new PoP_Frontend_ResourceLoaderMappingParser();
