...
class PoP_ResourceLoader_ConfigFileGeneratorBase extends PoP_Engine_RendererFileGeneratorBase {
	...
	function get_dir() {
		...
		return POP_FRONTENDENGINE_CONTENT_DIR.'/resourceloader/'.pop_version().'/'.$vars['theme'].'/'.$vars['thememode'];
	}
	function get_url() {

		$vars = GD_TemplateManager_Utils::get_vars();
		return POP_FRONTENDENGINE_CONTENT_URI.'/resourceloader/'.pop_version().'/'.$vars['theme'].'/'.$vars['thememode'];
	}
}