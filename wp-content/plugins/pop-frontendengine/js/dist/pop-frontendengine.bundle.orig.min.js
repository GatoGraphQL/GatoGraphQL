...
if(count){target.removeClass('hidden').text(count);if(updateTitle){document.title='('+count+') '+t.documentTitle;}}}},initMultiDomainFeedback:function(domain,pageSection,block){var t=this;var memory=t.getMemory(domain);var localDomain=t.getBlockTopLevelDomain(block);if(domain!=localDomain){var pssId=t.getSettingsId(pageSection);var bsId=t.getSettingsId(block);var localMemory=t.getMemory(localDomain);if($.isEmptyObject(memory.feedback.toplevel)){memory.feedback.toplevel=$.extend({},localMemory.feedback.toplevel);if(typeof memory.feedback.toplevel[M.DATALOAD_USER]!='undefined'){delete memory.feedback.toplevel[M.DATALOAD_USER];}}
if($.isEmptyObject(memory.feedback.pagesection[pssId])){memory.feedback.pagesection[pssId]=$.extend({},localMemory.feedback.pagesection[pssId]);memory.feedback.block[pssId]={};}
if($.isEmptyObject(memory.feedback.block[pssId][bsId])){memory.feedback.block[pssId][bsId]=$.extend({},localMemory.feedback.block[pssId][bsId]);}}},initMultiDomainMemory:function(domain,pageSection,block,response){var t=this;var memory=t.getMemory(domain);var localDomain=popManager.getBlockTopLevelDomain(block);if(domain!=localDomain){var localMemory=t.getMemory(localDomain);if($.isEmptyObject(memory.dataset)){memory.runtimesettings={'query-url':{},'query-multidomain-urls':{},configuration:{},'js-settings':{},};memory.settings={'js-settings':{},jsmethods:{pagesection:{},block:{},},'templates-cbs':{},'templates-paths':{},'db-keys':{},configuration:{},};}
