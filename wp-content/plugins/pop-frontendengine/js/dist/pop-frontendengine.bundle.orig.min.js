...
return mempage[pssId][targetId];},isFirstLoad:function(pageSection){var t=this;return t.firstLoad[popManager.getSettingsId(pageSection)];},init:function(){var t=this;$.each(M.ALLOWED_DOMAINS,function(index,domain){t.state[domain]={memory:{settings:{'js-settings':{},jsmethods:{pagesection:{},block:{},},'templates-cbs':{},'templates-paths':{},'db-keys':{},configuration:{},},runtimesettings:{'query-url':{},'query-multidomain-urls':{},configuration:{},'js-settings':{},},dataset:{},feedback:{block:{},pagesection:{},toplevel:{}},'query-state':{general:{},domain:{},}},database:{},userdatabase:{},};});var domain=M.HOME_DOMAIN;if($('#'+popPageSectionManager.getTopLevelSettingsId()).length){t.initDocument(domain);t.initDomain(domain,false);t.initTopLevelJson(domain);t.initLocalStorage();popUserAccount.initialLogin(domain);var memory=t.getMemory(domain);var pageSections=[];$.each(memory.settings.configuration,function(pssId,psConfiguration){var psId=psConfiguration[M.JS_FRONTENDID];var pageSection=$('#'+psId);pageSection.attr('data-settings-id',pssId);pageSection.addClass('pop-pagesection');t.initPageSectionSettings(domain,pageSection,psConfiguration);t.addPageSectionIds(domain,pageSection,psConfiguration[M.JS_TEMPLATE]);t.initPageSection(pageSection);pageSections.push(pageSection);});var options={'serverside-rendering':M.USESERVERSIDERENDERING}
...
var status=t.getMultiDomainBlockStatus(blockQueryState,fetch_urls,timestamp);var lastPromise=t.mergingTemplatePromise[domain];var dfd=$.Deferred();var thisPromise=dfd.promise();t.mergingTemplatePromise[domain]=thisPromise;if(lastPromise){lastPromise.done(function(){t.executeFetchBlockSuccess(pageSection,block,response,status,jqXHR);});}
else{t.executeFetchBlockSuccess(pageSection,block,response,status,jqXHR);}
...
var lastPromise=t.mergingTemplatePromise[domain];var dfd=$.Deferred();var thisPromise=dfd.promise();t.mergingTemplatePromise[domain]=thisPromise;if(lastPromise){lastPromise.done(function(){try{t.fetchBlockComplete(domain,pageSection,block,status,options);}
catch(err){console.error(err.message);}});}
...
catch(err){console.error(err.message);}});});block.triggerHandler('fetched',[response,action,domain]);},triggerEvent:function(pageSection,block,event,args){var t=this;block.triggerHandler(event,args);},integrateTopLevelFeedback:function(domain,response){var t=this;var tlFeedback=t.getMemory(domain).feedback.toplevel;$.each(response.feedback.toplevel,function(key,value){if($.type(value)=='array'&&value.length==0){}
