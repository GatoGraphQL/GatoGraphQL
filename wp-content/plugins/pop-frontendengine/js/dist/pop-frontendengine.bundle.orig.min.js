...
var dfd=$.Deferred();t.mergingTemplatePromise=dfd.promise();$.each(pageSections,function(index,pageSection){t.renderPageSection(domain,pageSection,options);});dfd.resolve();$(document.body).removeClass('pop-loadingframe');$.each(pageSections,function(index,pageSection){t.pageSectionInitialized(domain,pageSection);});$(document.body).removeClass('pop-loadingjs');var topLevelFeedback=t.getTopLevelFeedback(domain);var url=topLevelFeedback[M.URLPARAM_URL];if(!topLevelFeedback[M.URLPARAM_SILENTDOCUMENT]){popBrowserHistory.replaceState(url);t.updateDocument(domain);}
...
var status=t.getMultiDomainBlockStatus(blockQueryState,fetch_urls,timestamp);var lastPromise=t.mergingTemplatePromise;var dfd=$.Deferred();var thisPromise=dfd.promise();t.mergingTemplatePromise=thisPromise;lastPromise.done(function(){try{t.executeFetchBlockSuccess(pageSection,block,response,status,jqXHR);}
catch(err){console.log('Error: '+err.message);}
...
var lastPromise=t.mergingTemplatePromise;var dfd=$.Deferred();var thisPromise=dfd.promise();t.mergingTemplatePromise=thisPromise;lastPromise.done(function(){try{t.fetchBlockComplete(domain,pageSection,block,status,options);}
catch(err){console.log('Error: '+err.message);}
...
memory.runtimesettings['js-settings'][rpssId]={};memory.settings['js-settings'][rpssId]={};memory.settings.jsmethods.pagesection[rpssId]=$.extend({},localMemory.settings.jsmethods.pagesection[rpssId]);memory.settings.jsmethods.block[rpssId]={};memory.settings['templates-cbs'][rpssId]={};memory.settings['templates-paths'][rpssId]={};memory.settings['db-keys'][rpssId]={};memory.settings.configuration[rpssId]={};memory.settings.configuration[rpssId][M.JS_MODULES]={};var psConfiguration=memory.settings.configuration[rpssId];var lpsConfiguration=localMemory.settings.configuration[rpssId];$.each(lpsConfiguration,function(key,value){if(key==M.JS_MODULES||key=='modules')return;if(key=='root-context'||key=='parent-context')return;t.copyToConfiguration(key,value,psConfiguration,true);});}
$.each(rpsParams,function(rbsId,rbParams){if($.isEmptyObject(memory['query-state'].general[rpssId][rbsId])){memory['query-state'].general[rpssId][rbsId]=$.extend({},localMemory['query-state'].general[rpssId][rbsId]);memory['query-state'].domain[rpssId][rbsId]=$.extend({},localMemory['query-state'].domain[rpssId][rbsId]);memory.dataset[rpssId][rbsId]=$.extend({},localMemory.dataset[rpssId][rbsId]);memory.runtimesettings.configuration[rpssId][rbsId]={};memory.runtimesettings['query-url'][rpssId][rbsId]=$.extend({},localMemory.runtimesettings['query-url'][rpssId][rbsId]);memory.runtimesettings['query-multidomain-urls'][rpssId][rbsId]=$.extend({},localMemory.runtimesettings['query-multidomain-urls'][rpssId][rbsId]);memory.runtimesettings.configuration[rpssId][rbsId]=$.extend({},localMemory.runtimesettings.configuration[rpssId][rbsId]);memory.runtimesettings['js-settings'][rpssId][rbsId]=$.extend({},localMemory.runtimesettings['js-settings'][rpssId][rbsId]);memory.settings['js-settings'][rpssId][rbsId]=$.extend({},localMemory.settings['js-settings'][rpssId][rbsId]);memory.settings.jsmethods.block[rpssId][rbsId]=$.extend({},localMemory.settings.jsmethods.block[rpssId][rbsId]);memory.settings['templates-cbs'][rpssId][rbsId]=$.extend({},localMemory.settings['templates-cbs'][rpssId][rbsId]);memory.settings['templates-paths'][rpssId][rbsId]=$.extend({},localMemory.settings['templates-paths'][rpssId][rbsId]);memory.settings['db-keys'][rpssId][rbsId]=$.extend({},localMemory.settings['db-keys'][rpssId][rbsId]);var bConfiguration=localMemory.settings.configuration[rpssId][M.JS_MODULES][rbsId];delete bConfiguration['root-context'];delete bConfiguration['parent-context'];memory.settings.configuration[rpssId][M.JS_MODULES][rbsId]=$.extend(true,{},bConfiguration);}});});}},executeFetchBlockSuccess:function(pageSection,block,response,status,jqXHR){var t=this;var blockQueryState=t.getBlockQueryState(pageSection,block);var loadingUrl=blockQueryState.url[jqXHR.url];var action=blockQueryState.action[loadingUrl];var runtimeOptions={url:blockQueryState['paramsscope-url'][loadingUrl]};var processOptions={operation:blockQueryState.operation[loadingUrl],action:blockQueryState.action[loadingUrl],'fetch-status':status};var domain=blockQueryState.domain[loadingUrl];t.initMultiDomainMemory(domain,pageSection,block,response);var memory=t.getMemory(domain);var restoreinitial_actions=[M.CBACTION_LOADCONTENT,M.CBACTION_REFETCH,M.CBACTION_RESET];var restoreinitial=restoreinitial_actions.indexOf(action)>-1;if(restoreinitial){var initialMemory=t.getInitialBlockMemory(runtimeOptions.url);$.each(response['query-state'].general,function(rpssId,rpsParams){$.each(rpsParams,function(rbsId,rbParams){memory.runtimesettings.configuration[rpssId][rbsId]=initialMemory.runtimesettings.configuration[rpssId][rbsId];});});}
...
var html=t.getTemplateHtml(domain,pageSection,target,templateName,options);var targetContainer=t.getMergeTarget(target,templateName,options);options.operation=options.operation||M.URLPARAM_OPERATION_REPLACE;if(options.operation==M.URLPARAM_OPERATION_REPLACE){target.triggerHandler('replace',[options.action]);if(rerender){target.triggerHandler('rerender',[options.action]);}
targetContainer.empty();}
var merged=t.mergeHtml(html,targetContainer,options.operation);target.triggerHandler('merged',[merged.newDOMs]);return merged;},mergeHtml:function(html,container,operation){var t=this;var newDOMs=$(html);if(operation==M.URLPARAM_OPERATION_PREPEND){container.prepend(newDOMs);}
