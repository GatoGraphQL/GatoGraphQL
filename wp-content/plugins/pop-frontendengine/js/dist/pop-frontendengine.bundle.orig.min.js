...
catch(err){console.log(err.message);}});});block.triggerHandler('fetched',[response,action]);},triggerEvent:function(pageSection,block,event,args){var t=this;block.triggerHandler(event,args);},integrateTopLevelFeedback:function(response){var t=this;var tlFeedback=t.getMemory().feedback.toplevel;$.each(response.feedback.toplevel,function(key,value){if($.type(value)=='array'&&value.length==0){}
