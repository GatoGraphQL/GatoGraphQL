...
return ret;}};})(jQuery);;(function($){popJSRuntimeManager={'full-session-ids':{},'last-session-ids':{},blockURL:null,pageSectionURL:'general',setBlockURL:function(url){var t=this;t.blockURL=url;},initBlockVarPaths:function(vars,url,pssId,targetId,template,group){var t=this;group=group||M.JSMETHOD_GROUP_MAIN;if(!vars[url]){vars[url]={};}
if(!vars[url][pssId]){vars[url][pssId]={};}
if(!vars[url][pssId][targetId]){vars[url][pssId][targetId]={};}
if(!vars[url][pssId][targetId][template]){vars[url][pssId][targetId][template]={};}
if(!vars[url][pssId][targetId][template][group]){vars[url][pssId][targetId][template][group]=[];}},initVars:function(url,pssId,targetId,template,group){var t=this;t.initBlockVarPaths(t['full-session-ids'],url,pssId,targetId,template,group);t.initBlockVarPaths(t['last-session-ids'],url,pssId,targetId,template,group);},addGroup:function(id,group){var t=this;if(group){id+=M.ID_SEPARATOR+group;}
return id;},addPageSectionId:function(pssId,template,id,group){var t=this;return t.addTemplateId(pssId,pssId,template,id,group,true,true);},addTemplateId:function(pssId,targetId,template,id,group,fixed,isIdUnique,ignorePSRuntimeId){var t=this;if(!isIdUnique){id+=popManager.getUniqueId();}
if(!fixed){id+=M.ID_SEPARATOR+counterNext();}
id=t.addGroup(id,group);if(!ignorePSRuntimeId){t.addTargetId(t.pageSectionURL,pssId,pssId,template,group,id);}
if(pssId!=targetId){t.addTargetId(t.blockURL,pssId,targetId,template,group,id);}
return id;},addTargetId:function(url,pssId,targetId,template,group,id){var t=this;group=group||M.JSMETHOD_GROUP_MAIN;t.initVars(url,pssId,targetId,template,group);t['full-session-ids'][url][pssId][targetId][template][group].push(id);t['last-session-ids'][url][pssId][targetId][template][group].push(id);return id;},getLastGeneratedId:function(pssId,targetId,template,group){var t=this;group=group||M.JSMETHOD_GROUP_MAIN;var url=(pssId==targetId)?t.pageSectionURL:t.blockURL;var ids=t['full-session-ids'][url][pssId][targetId][template][group];return ids[ids.length-1];},getPageSectionSessionIds:function(pageSection,session){var t=this;var url=t.pageSectionURL;return t.getTargetSessionIds(url,pageSection,pageSection,session);},getBlockSessionIds:function(pageSection,block,session){var t=this;var url=t.getBlockURL(pageSection,block);return t.getTargetSessionIds(url,pageSection,block,session);},getTargetSessionIds:function(url,pageSection,target,session){var t=this;var pssId=popManager.getSettingsId(pageSection);var targetId=popManager.getSettingsId(target);session=session||'last';if(t[session+'-session-ids'][url]&&t[session+'-session-ids'][url][pssId]){return t[session+'-session-ids'][url][pssId][targetId];}
return null;},deletePageSectionLastSessionIds:function(pageSection){var t=this;var pssId=popManager.getSettingsId(pageSection);var url=t.pageSectionURL;delete t['last-session-ids'][url][pssId][pssId];if($.isEmptyObject(t['last-session-ids'][url][pssId])){delete t['last-session-ids'][url][pssId];if($.isEmptyObject(t['last-session-ids'][url])){delete t['last-session-ids'][url];}}},deleteBlockLastSessionIds:function(pageSection,block){var t=this;var pssId=popManager.getSettingsId(pageSection);var targetId=popManager.getSettingsId(block);var url=t.getBlockURL(pageSection,block);if(t['last-session-ids'][url]&&t['last-session-ids'][url][pssId]&&t['last-session-ids'][url][pssId][targetId]){delete t['last-session-ids'][url][pssId][targetId];if($.isEmptyObject(t['last-session-ids'][url][pssId])){delete t['last-session-ids'][url][pssId];if($.isEmptyObject(t['last-session-ids'][url])){delete t['last-session-ids'][url];}}}},getBlockURL:function(pageSection,block){var t=this;return block.data('toplevel-url');},};})(jQuery);;function showTopNav(){popPageSectionManager.open(popPageSectionManager.getTopNav());}
...
$.each(pageSections,function(index,pageSection){t.renderPageSection(pageSection,options);});$(document.body).removeClass('pop-loadingframe');$.each(pageSections,function(index,pageSection){t.pageSectionInitialized(pageSection);});$(document.body).removeClass('pop-loadingjs');var topLevelFeedback=t.getTopLevelFeedback();var url=topLevelFeedback[M.URLPARAM_URL];if(!topLevelFeedback[M.URLPARAM_SILENTDOCUMENT]){popBrowserHistory.replaceState(url);t.updateDocument();}
t.documentInitialized();t.backgroundLoad(M.BACKGROUND_LOAD);t.backgroundLoad(topLevelFeedback[M.URLPARAM_BACKGROUNDLOADURLS]);}},expandJSKeys:function(context){var t=this;if(context&&M.COMPACT_JS_KEYS){if(context[M.JS_MODULES]){context.modules=context[M.JS_MODULES];}
...
var args={pageSection:pageSection,newDOMs:newDOMs};t.extendArgs(args,options);popJSLibraryManager.execute('pageSectionNewDOMsBeforeInitialize',args);t.initPageSectionBranches(pageSection,newDOMs,options);popJSLibraryManager.execute('pageSectionNewDOMsInitialized',args);},initPageSectionBranches:function(pageSection,newDOMs,options){var t=this;t.runPageSectionJSMethods(pageSection,options);var jsSettings=popManager.getPageSectionJsSettings(pageSection);var blockBranches=jsSettings['initjs-blockbranches'];if(blockBranches){var branches=$(blockBranches.join(',')).not('.'+M.JS_INITIALIZED);t.initBlockBranches(pageSection,branches,options);}
popJSRuntimeManager.deletePageSectionLastSessionIds(pageSection);},triggerDestroyTarget:function(url,target){var t=this;target=target||M.URLPARAM_TARGET_MAIN;t.removeOpenTab(url,target);popURLInterceptors.intercept(t.getDestroyUrl(url),{target:target});},destroyTarget:function(pageSection,target){var t=this;var args={pageSection:pageSection,destroyTarget:target}
...
t.storeData('PoP:openTabs',tabs);return true;},replaceOpenTab:function(fromURL,toURL,target){var t=this;if(!M.KEEP_OPEN_TABS)return;if(t.removeOpenTab(fromURL,target)){t.addOpenTab(toURL,target);}},getStoredData:function(localStorageKey,use_version){var t=this;if(M.USELOCALSTORAGE&&Modernizr.localstorage){var stored=localStorage[localStorageKey];if(stored){stored=$.parseJSON(stored);if(use_version){if((stored.version==M.VERSION)&&(typeof stored.expires=='undefined'||stored.expires>Date.now())){return stored.value;}
delete localStorage[localStorageKey];return null;}
else{return stored.value;}}}
return null;},storeData:function(localStorageKey,value,expires){var t=this;if(M.USELOCALSTORAGE&&Modernizr.localstorage){var stored={version:M.VERSION,value:value}
...
options=options||{};var target=t.getTarget(pageSection);var args={options:options,url:url};popJSLibraryManager.execute('modifyFetchArgs',args);var fetchUrl=args.url;if(fetchUrl.indexOf('#')>-1){fetchUrl=fetchUrl.substr(0,fetchUrl.indexOf('#'));}
...
var crossdomain=t.getCrossDomainOptions(fetchUrl);$.ajax($.extend(crossdomain,{dataType:"json",url:fetchUrl,data:postData,beforeSend:function(jqXHR,settings){jqXHR.url=url;params.url[jqXHR.url]=url;params.target[jqXHR.url]=target;params['fetch-url'][jqXHR.url]=settings.url;params.loading.push(url);t.handleLoadingStatus(pageSection,'add',options);},complete:function(jqXHR){var url=params.url[jqXHR.url];delete params.url[jqXHR.url];delete params.target[jqXHR.url];delete params['fetch-url'][jqXHR.url];params.loading.splice(params.loading.indexOf(url),1);t.handleLoadingStatus(pageSection,'remove',options);if(options['urlfetched-callbacks']){var handler='urlfetched:'+escape(url);$.each(options['urlfetched-callbacks'],function(index,callback){$(document).one(handler,callback);});}
t.triggerURLFetched(url);if(options['disable-layer']){options['disable-layer'].children('.pop-disabledlayer').addClass('hidden');}},success:function(response,textStatus,jqXHR){t.pageSectionFetchSuccess(pageSection,response);t.backgroundLoad(response.feedback.toplevel[M.URLPARAM_BACKGROUNDLOADURLS]);error.addClass('hidden');var url=params.url[jqXHR.url];var feedbackURL=response.feedback.toplevel[M.URLPARAM_URL];var target=params.target[jqXHR.url];if(url!=feedbackURL){t.replaceOpenTab(url,feedbackURL,target);}
...
return post_data;},handleBlockError:function(error,jqXHR,options){var t=this;var msgSelectors=['.errormsg.status-'+jqXHR.status,'.errormsg.general'];$.each(msgSelectors,function(index,msgSelector){var msg=error.find(msgSelector);if(msg.length){msg.removeClass('hidden').siblings('.errormsg').addClass('hidden');return false;}});if(!options['skip-status']){error.removeClass('hidden');t.scrollTop(error);}},executeFetchBlock:function(pageSection,block,options){var t=this;options=options||{};var fetchUrl=t.getQueryUrl(pageSection,block);if(!fetchUrl){return;}
var type=options.type||'GET';var args={options:options,url:fetchUrl,type:type};popJSLibraryManager.execute('modifyFetchBlockArgs',args);fetchUrl=args.url;var params=t.getBlockParams(pageSection,block);if(options['post-data']){params['post-data']=options['post-data'];}
...
var crossdomain=t.getCrossDomainOptions(fetchUrl);$.ajax($.extend(crossdomain,{dataType:"json",url:fetchUrl,data:post_data,type:type,beforeSend:function(jqXHR,settings){jqXHR.url=settings.url;params.url[jqXHR.url]=loadingUrl;params.loading.push(loadingUrl);params.operation[loadingUrl]=options.operation;params.action[loadingUrl]=options.action;params['paramsscope-url'][loadingUrl]=block.data('paramsscope-url');if(options.reload){params.reload.push(loadingUrl);}},success:function(response,textStatus,jqXHR){t.blockFetchSuccess(pageSection,block,response);t.backgroundLoad(response.feedback.toplevel[M.URLPARAM_BACKGROUNDLOADURLS]);if(options['hide-latestcount']){block.children('.blocksection-latestcount').find('.pop-latestcount').addClass('hidden').find('.pop-count').text('0');}
...
dfd.resolve();},error:function(jqXHR,textStatus,errorThrown){t.handleBlockError(error,jqXHR,options);t.triggerEvent(pageSection,block,'fetchFailed');},complete:function(jqXHR){var status={};var loadingUrl=params.url[jqXHR.url];delete params.url[jqXHR.url];params.loading.splice(params.loading.indexOf(loadingUrl),1);delete params.operation[loadingUrl];delete params.action[loadingUrl];delete params['paramsscope-url'][loadingUrl];var index=params.reload.indexOf(loadingUrl);if(index>-1){status.reload=true;params.reload.splice(index,1);}
var lastPromise=t.mergingTemplatePromise;var dfd=$.Deferred();var thisPromise=dfd.promise();t.mergingTemplatePromise=thisPromise;if(lastPromise){lastPromise.done(function(){try{t.fetchBlockComplete(pageSection,block,params,status,options);}
catch(err){console.error(err.message);}});}
else{try{t.fetchBlockComplete(pageSection,block,params,status,options);}
catch(err){console.error(err.message);}}
...
catch(err){console.error(err.message);}});});block.triggerHandler('fetched',[response,action]);},triggerEvent:function(pageSection,block,event,args){var t=this;block.triggerHandler(event,args);},integrateTopLevelFeedback:function(response){var t=this;var tlFeedback=t.getMemory().feedback.toplevel;$.each(response.feedback.toplevel,function(key,value){if($.type(value)=='array'&&value.length==0){}
...
return'';},getItemObject:function(itemDBKey,itemObjectId){var t=this;var userItem={},item={};if(t.userdatabase[itemDBKey]&&t.userdatabase[itemDBKey][itemObjectId]){userItem=t.userdatabase[itemDBKey][itemObjectId];}
