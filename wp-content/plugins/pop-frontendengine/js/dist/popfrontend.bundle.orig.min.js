...
return;}
t.fetchPageSection(pageSection,url,options);},initLocalStorage:function(){if(M.USELOCALSTORAGE&&Modernizr.localstorage){var latest=localStorage['PoP:version'];if(!latest||(latest!=M.VERSION)){Object.keys(localStorage).forEach(function(key){if(key.startsWith(M.HOME_URL)){localStorage.removeItem(key);}});localStorage['PoP:version']=M.VERSION;}}},openTabs:function(){var t=this;var options={silentDocument:true,'js-args':{inactivePane:true,addOpenTab:false}};var tabs=t.getOpenTabs();$.each(tabs,function(target,urls){options.target=target;if(target==M.URLPARAM_TARGET_MAIN){var pos=urls.indexOf(window.location.href);if(pos!==-1){urls.splice(pos,1);}}
$.each(urls,function(index,url){t.fetch(url,options);});});},getOpenTabs:function(){var t=this;return t.getStoredData('PoP:openTabs')||{};},addOpenTab:function(url,target){var t=this;var tabs=t.getOpenTabs();tabs[target]=tabs[target]||[];if(tabs[target].indexOf(url)>-1){return false;}
tabs[target].push(url);t.storeData('PoP:openTabs',tabs);return true;},removeOpenTab:function(url,target){var t=this;var tabs=t.getOpenTabs();tabs[target]=tabs[target]||[];var pos=tabs[target].indexOf(url);if(pos===-1){return false;}
tabs[target].splice(pos,1);t.storeData('PoP:openTabs',tabs);return true;},replaceOpenTab:function(fromURL,toURL,target){var t=this;if(t.removeOpenTab(fromURL,target)){t.addOpenTab(toURL,target);}},getStoredData:function(localStorageKey){var t=this;if(M.USELOCALSTORAGE&&Modernizr.localstorage){var stored=localStorage[localStorageKey];if(stored){stored=$.parseJSON(stored);if((stored.version==M.VERSION)&&(typeof stored.expires=='undefined'||stored.expires>Date.now())){return stored.value;}
