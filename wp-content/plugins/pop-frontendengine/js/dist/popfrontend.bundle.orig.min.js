...
return;}
t.fetchPageSection(pageSection,url,options);},initLocalStorage:function(){if(M.USELOCALSTORAGE&&Modernizr.localstorage){var latest=localStorage['PoP:version'];if(!latest||(latest!=M.VERSION)){Object.keys(localStorage).forEach(function(key){if(key.startsWith(M.HOME_URL)){localStorage.removeItem(key);}});localStorage['PoP:version']=M.VERSION;}}},getStoredData:function(localStorageKey){if(M.USELOCALSTORAGE&&Modernizr.localstorage){var stored=localStorage[localStorageKey];if(stored){stored=$.parseJSON(stored);if((stored.version==M.VERSION)&&(typeof stored.expires=='undefined'||stored.expires>Date.now())){return stored.value;}
