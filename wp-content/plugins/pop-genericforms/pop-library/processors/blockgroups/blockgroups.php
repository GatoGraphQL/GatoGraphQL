	...
	function get_title($template_id) {
		...
		switch ($template_id) {
			...
			case GD_TEMPLATE_BLOCKGROUP_VOLUNTEER:
				...
				return gd_navigation_menu_item(POP_GENERICFORMS_PAGE_VOLUNTEER, true).__('Volunteer', 'pop-genericforms');

			case GD_TEMPLATE_BLOCKGROUP_FLAG:

				return gd_navigation_menu_item(POP_GENERICFORMS_PAGE_FLAG, true).__('Flag as inappropriate', 'pop-genericforms');

			case GD_TEMPLATE_BLOCKGROUP_CONTACTUSER:

				return gd_navigation_menu_item(POP_GENERICFORMS_PAGE_CONTACTUSER, true).__('Send message', 'pop-genericforms');
		}
		...
		switch ($blockgroup) {
			...
			case GD_TEMPLATE_BLOCKGROUP_VOLUNTEER:
				...
				elseif ($blockgroup_block == GD_TEMPLATE_BLOCK_POSTHEADER) {
					...
					$description = __('Volunteer for:', 'pop-genericforms');
			...
			case GD_TEMPLATE_BLOCKGROUP_FLAG:
				...
				elseif ($blockgroup_block == GD_TEMPLATE_BLOCK_POSTHEADER) {
					...
					$description = __('Flag as inappropriate:', 'pop-genericforms');
					$this->add_att($blockgroup_block, $blockgroup_block_atts, 'description', $description);
				}
				break;
			...
			case GD_TEMPLATE_BLOCKGROUP_CONTACTUSER:
				...
				elseif ($blockgroup_block == GD_TEMPLATE_BLOCK_USERHEADER) {
					...
					$description = __('Send a message to:', 'pop-genericforms');
					$this->add_att($blockgroup_block, $blockgroup_block_atts, 'description', $description);
				}
				break;
		}
