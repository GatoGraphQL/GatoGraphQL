...
/**---------------------------------------------------------------------------------------------------------------
 *
 * Implementation taken from "User Avatar" version 1.4.11 (http://wordpress.org/extend/plugins/user-avatar/), from Enej Bajgoric / Gagan Sandhu / CTLT DEV
 *
 * ---------------------------------------------------------------------------------------------------------------*/
...
function user_avatar_core_set_avatar_constants() {
	...
	if (!defined('POP_USERAVATAR_USEMETA')) {
		define ('POP_USERAVATAR_USEMETA', true);
	}
...
function gd_useravatar_delete_filename($uid) {
	...
	delete_user_meta($uid, POP_USERAVATAR_FILENAME);
}
...
function gd_useravatar_save_filename($uid, $filename) {
	...
	add_user_meta($uid, POP_USERAVATAR_FILENAME, $filename, true);
}
...
function gd_useravatar_get_filename($uid) {
	...
	// Save the user meta with the avatar filename
	return get_user_meta($uid, POP_USERAVATAR_FILENAME, true);
}
