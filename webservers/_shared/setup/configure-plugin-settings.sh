...
curl -i --insecure \
  ...
  -X POST \
  -H "Content-Type: application/json" \
  -d '{"jsonEncodedOptionValues": "{\"custompost-types\":[\"dummy-cpt\",\"dummy-cpt-two\",\"dummy-cpt-three\",\"dummy-cpt-four\",\"page\",\"post\",\"wp_block\"]}"}' \
...
# Disable "sensitive fields" for Meta interface as to test disabling for each particular entity (taxonomies, etc) without this option interfering
curl -i --insecure \
  --user "admin:$(echo $ADMIN_USER_APP_PASSWORD)" \
  -X POST \
  -H "Content-Type: application/json" \
  -d '{"jsonEncodedOptionValues": "{\"treat-meta-keys-as-sensitive-data\":false}"}' \
  $(echo $SITE_DOMAIN)/wp-json/gatographql/v1/admin/module-settings/gatographql_gatographql_schema-meta
