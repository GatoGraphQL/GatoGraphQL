...
wp menu item add-custom bottom-menu BBC https://bbc.com --parent-id=$(wp menu item add-post bottom-menu 1 --title="Parent post in Menu for testing" --parent-id=$(wp menu item add-post bottom-menu 1 --title="Most ancestor post in Menu for testing" --porcelain --path=/app/wordpress) --porcelain --path=/app/wordpress) --path=/app/wordpress
