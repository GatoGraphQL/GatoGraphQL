#!/bin/sh
wp core install --url="gato-graphql-for-prod.lndo.site" --title="Gato GraphQL for PROD (PHP 7.1 + Generated .zip plugins)" --admin_user=admin --admin_password=admin --admin_email=admin@example.com --path=/app/wordpress 
