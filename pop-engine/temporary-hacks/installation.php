...
class Temporary_Hacks_Installation {
	...
	function __construct() {
		...
		if ($_REQUEST['action'] == 'install-hack') {
			...
			$plugin_version = $this->system_activateplugins(array());
			foreach ($plugin_version as $plugin => $activate_version) {
				...
				$this->run_activate_plugin("${plugin}/${plugin}.php");
			}

			// Execute the activate method for plugins which are already activate
			add_action('init', array($this, 'reactivate_plugins'));
		}
	}

	function reactivate_plugins() {
	...
	function system_activateplugins($plugin_version) {
