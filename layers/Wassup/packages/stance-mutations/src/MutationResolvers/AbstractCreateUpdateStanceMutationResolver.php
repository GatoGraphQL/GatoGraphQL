...
abstract class AbstractCreateUpdateStanceMutationResolver extends AbstractCreateUpdateCustomPostMutationResolver
{
    ...
    protected function validateContent(array &$errors, WithArgumentsInterface $withArgumentsAST): void
    {
        if ($withArgumentsAST->getArgumentValue('stancetarget')) {
            ...
            $referenced = $this->getCustomPostTypeAPI()->getCustomPost($withArgumentsAST->getArgumentValue('stancetarget'));
            if (!$referenced) {
    ...
    {
        parent::validateCreateContent($errors, $withArgumentsAST);
        ...
        $referenced_id = $withArgumentsAST->getArgumentValue('stancetarget');
    ...
    protected function createAdditionals(string | int $post_id, WithArgumentsInterface $withArgumentsAST): void
    {
        ...
        if ($target = $withArgumentsAST->getArgumentValue('stancetarget')) {
            Utils::addCustomPostMeta($post_id, GD_METAKEY_POST_STANCETARGET, $target, true);
        }

        // Allow for URE to add the AuthorRole meta value
        App::doAction('GD_CreateUpdate_Stance:createAdditionals', $post_id, $withArgumentsAST);
    }
    ...
    protected function updateAdditionals(string | int $post_id, WithArgumentsInterface $withArgumentsAST, array $log): void
    {
        ...
        // Allow for URE to add the AuthorRole meta value
        App::doAction('GD_CreateUpdate_Stance:updateAdditionals', $post_id, $withArgumentsAST, $log);
    }
}
