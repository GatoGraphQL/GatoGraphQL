...
class EditMembershipMutationResolverBridge extends AbstractComponentMutationResolverBridge
{
    ...
    public function appendMutationDataToFieldDataAccessor(\PoP\ComponentModel\Mutation\FieldDataAccessorInterface $fieldDataAccessor): void
    {
        ...
        $fieldDataAccessor->add('community', $community);
        $fieldDataAccessor->add('user_id', App::query(InputNames::USER_ID));
        // $fieldDataAccessor->add('nonce', App::query(POP_INPUTNAME_NONCE));
        $fieldDataAccessor->add('status', trim($this->getComponentProcessorManager()->getComponentProcessor([GD_URE_Module_Processor_SelectFormInputs::class, GD_URE_Module_Processor_SelectFormInputs::COMPONENT_URE_FORMINPUT_MEMBERSTATUS])->getValue([GD_URE_Module_Processor_SelectFormInputs::class, GD_URE_Module_Processor_SelectFormInputs::COMPONENT_URE_FORMINPUT_MEMBERSTATUS])));
        $fieldDataAccessor->add('privileges', $privileges ?? array());
        $fieldDataAccessor->add('tags', $tags ?? array());
    }
}
