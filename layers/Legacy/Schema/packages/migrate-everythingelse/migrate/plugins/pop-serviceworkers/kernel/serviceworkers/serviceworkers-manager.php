...
class PoP_ServiceWorkersManager
{
    ...
    private function manifest()
    {
        ...
        if ($short_name = \PoP\Root\App::getHookManager()->applyFilters('PoP_ServiceWorkersManager:manifest:short_name', $blogname)) {
            $json['short_name'] = $short_name;
        }
        if ($description = \PoP\Root\App::getHookManager()->applyFilters('PoP_ServiceWorkersManager:manifest:description', $description)) {
            $json['description'] = $description;
        }
        if ($name = \PoP\Root\App::getHookManager()->applyFilters('PoP_ServiceWorkersManager:manifest:name', $blogname)) {
            $json['name'] = $name;
        }
        if ($icons = \PoP\Root\App::getHookManager()->applyFilters('PoP_ServiceWorkersManager:manifest:icons', array())) {
            $json['icons'] = $icons;
        }
        if ($start_url = \PoP\Root\App::getHookManager()->applyFilters('PoP_ServiceWorkersManager:manifest:start_url', $cmsService->getSiteURL())) {
            $json['start_url'] = $start_url;
        }
        if ($display = \PoP\Root\App::getHookManager()->applyFilters('PoP_ServiceWorkersManager:manifest:display', 'standalone')) {
            $json['display'] = $display;
        }
        if ($orientation = \PoP\Root\App::getHookManager()->applyFilters('PoP_ServiceWorkersManager:manifest:orientation', 'portrait')) {
            $json['orientation'] = $orientation;
        }
        if ($theme_color = \PoP\Root\App::getHookManager()->applyFilters('PoP_ServiceWorkersManager:manifest:theme_color', '#fff')) {
            $json['theme_color'] = $theme_color;
        }
        if ($background_color = \PoP\Root\App::getHookManager()->applyFilters('PoP_ServiceWorkersManager:manifest:background_color', '#fff')) {
