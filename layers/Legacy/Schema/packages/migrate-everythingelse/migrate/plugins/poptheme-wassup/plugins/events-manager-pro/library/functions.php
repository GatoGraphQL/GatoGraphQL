...
\PoP\Root\App::removeAction('show_user_profile', array('EM_User_Fields','show_profile_fields'), 1);
...
\PoP\Root\App::addAction('after_setup_theme', 'gdPluginsLoadedReorderGateways');
...
if (!is_admin()) {
    ...
    \PoP\Root\App::removeAction('em_events_admin_bookings_footer', array('EM_Coupons', 'admin_meta_box'), 20, 1);
    \PoP\Root\App::removeAction('em_events_admin_bookings_footer', array('EM_Booking_Form', 'event_bookings_meta_box'), 20, 1);

    // Gateway Offline
    global $EM_Gateways;
    if ($EM_Gateways && $EM_Gateways['offline']) {
        \PoP\Root\App::removeAction('em_admin_event_booking_options_buttons', array($EM_Gateways['offline'], 'event_booking_options_buttons'), 10);
        \PoP\Root\App::removeAction('em_admin_event_booking_options', array($EM_Gateways['offline'], 'event_booking_options'), 10);
        \PoP\Root\App::removeAction('em_bookings_single_metabox_footer', array($EM_Gateways['offline'], 'add_payment_form'), 1, 1); //add payment to booking
    }

    // Remove options to export or filter
    \PoP\Root\App::removeFilter('em_bookings_table_cols_template', array('EM_Booking_Form','em_bookings_table_cols_template'), 10, 2);
    \PoP\Root\App::removeAction('em_bookings_table_cols_template', array('EM_Coupons', 'em_bookings_table_cols_template'), 10, 1);

    global $EM_Gateways_Transactions;
    if ($EM_Gateways_Transactions) {
        \PoP\Root\App::removeFilter('em_bookings_table_cols_template', array($EM_Gateways_Transactions, 'em_bookings_table_cols_template'), 10, 2);
    }
}
