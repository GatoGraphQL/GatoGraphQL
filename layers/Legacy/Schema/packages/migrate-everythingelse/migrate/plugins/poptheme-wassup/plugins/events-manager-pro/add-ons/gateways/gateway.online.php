...
if (class_exists("EM_Gateway")) {
    class EM_Gateway_Online extends EM_Gateway
    {
        ...
        public function __construct()
        {
            ...
            if ($this->is_active()) {
                \PoP\Root\App::addAction('em_gateway_js', array($this,'emGatewayJs'));
                //Gateway-Specific
                \PoP\Root\App::addAction('em_template_my_bookings_header', array($this,'sayThanks')); //say thanks on my_bookings page
                \PoP\Root\App::addFilter('em_bookings_table_booking_actions_4', array($this,'bookingsTableActions'), 1, 2);
                \PoP\Root\App::addFilter('em_my_bookings_booking_actions', array($this,'emMyBookingsBookingActions'), 1, 2);
                //set up cron
        ...
        */
        ...
        public function bookingAdd($EM_Event, $EM_Booking, $post_validation = false)
        {
            ...
            if (!defined('DOING_AJAX')) { //we aren't doing ajax here, so we should provide a way to edit the $EM_Notices ojbect.
                \PoP\Root\App::addAction('option_dbem_booking_feedback', array($this, 'bookingFormFeedbackFallback'));
            }
        }
