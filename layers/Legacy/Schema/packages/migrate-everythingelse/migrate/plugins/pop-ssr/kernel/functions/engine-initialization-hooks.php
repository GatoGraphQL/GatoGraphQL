...
class PoP_SSR_EngineInitialization_Hooks
{
    ...
    protected function removeDatabases(&$data, $engine)
    {
        ...
        $entryComponent = $engine->getEntryComponent();
        $entryComponentOutputName = \PoP\ComponentModel\Facades\ComponentHelpers\ComponentHelpersFacade::getInstance()->getComponentOutputName($entryComponent);
        ...
        if ($dynamic_data_properties === null) {
            ...
            $dynamic_data_properties = $pop_component_processordynamicdatadecorator_manager->getProcessorDecorator($componentprocessor_manager->getComponentProcessor($entryComponent))->getDynamicDataFieldsDatasetcomponentTree($entryComponent, $entry_model_props);
    ...
    {
        if ($data_properties['direct-component-fields'] ?? null) {
            ...
            $engine = EngineFacade::getInstance();
            $data_fields = $engine->moveEntriesUnderDBName($data_properties['direct-component-fields'], true, $relationalTypeResolver);
