...
define('WPSCPOP_POP_CMSWP_MIN_VERSION', 0.1);

class WPSC_PoP_Validation
{
    public function validate()
    {
        ...
        if (!defined('POP_ENGINEWP_VERSION')) {
            ...
            $success = false;
        } elseif (WPSCPOP_POP_CMSWP_MIN_VERSION > POP_ENGINEWP_VERSION) {
        ...
        if (!(function_exists('wpsupercache_site_admin'))) {
            ...
            // Do not initialize the plug-in, but no need to show an error message
            $success = false;
        }
    ...
    public function initializeWarning()
    {
        $this->dependencyInitializationWarning(
            TranslationAPIFacade::getInstance()->__('WP Super Cache for PoP', 'wp-super-cache-pop'),
            TranslationAPIFacade::getInstance()->__('PoP Engine for WordPress', 'wp-super-cache-pop')
        );
    }
    ...
    public function pluginWarning()
    {
        $this->dependencyInstallationWarning(
            TranslationAPIFacade::getInstance()->__('WP Super Cache for PoP', 'wp-super-cache-pop'),
            TranslationAPIFacade::getInstance()->__('WP Super Cache', 'wp-super-cache-pop'),
