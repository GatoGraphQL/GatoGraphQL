...
Since meta value can be anything (string, integer, float, boolean, array, or null) they have been mapped via the generic scalar type `AnyScalar`.
...
It is used on the newly-introduced `metaValue(s)` and `option` fields, to acknowledge the fact that we do not know in advance the type of the data returned by these fields.

(This is because functions `get_option`, `get_post_meta`, `get_user_meta`, and others, can store data with of type: boolean, string, int, etc.)
...
Support for lists as fields types has been further expanded, now supporting the following features (as [defined by the GraphQL spec](https://spec.graphql.org/draft/#sec-List)):
...
The plugin now supports passing a single value where a list is expected, as [defined in the GraphQL spec](https://spec.graphql.org/draft/#sec-List.Input-Coercion).
...
The affected hooks have been replaced with the equivalent ones:
