...
**Important:** this release contains **breaking changes**! Scroll to the bottom to find out.
...
It is used on the newly-introduced `option` and `metaValue(s)` field, because we do not know in advance the type of their returned data, and the union of scalar types is currently not supported by the GraphQL spec.

---

## Breaking changes

Due to the widespread refactoring of the codebase, this new release produces breaking changes with the previous version, which affects the configuration saved for the plugin.

### Configuration breaking changes

Schema Configurations, Custom Endpoints and Persisted Queries have had their "Options" block rebuilt.

In the previous `v0.7`, a single Options block contained many configuration items. Since `v0.8`, this block has been decoupled into several independent blocks, each containing its own configuration items.

For instance, in addition to enabling/disabling the endpoint, the Custom Endpoint Options block allowed to configure the GraphiQL and Interactive Schema clients:
...
Now, this configuration is added through the GraphiQL and Interactive Schema blocks:

<a href="../../images/custom-endpoint.png" target="_blank">![Options in Custom Endpoint](../../images/custom-endpoint.png)</a>

The configuration stored in the Options blocks for all 3 CPTs (Schema Configurations, Custom Endpoints and Persisted Queries) is not automatically migrated to the new format.

Hence, before upgrading to `v0.8`, please write down your stored configuration, and replicate it after upgrading to the new version.
