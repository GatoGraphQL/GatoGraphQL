...
For instance, in addition to enabling/disabling the endpoint, in `v0.7` the Custom Endpoint Options block allowed to configure the GraphiQL and Interactive Schema clients:
...
Only the directives defined in the GraphQL spec are now shipped in the plugin:

- `@include`
- `@skip`

The other directives have been removed:

- `@default`
- `@removeIfNull`
- `@export` (along the "Multiple Query Execution" module)
