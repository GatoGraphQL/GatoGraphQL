...
It is used on the newly-introduced `metaValue`, `metaValues` and `option` fields, because we do not know in advance the type of the data returned by these fields, and the union of scalar types is currently not supported by the GraphQL spec.
