...
One use case for composable directives is to manipulate content from blocks (added via the WordPress editor, aka "Gutenberg"), for whichever block the post contains, as detailed in blog post [💬 Proposing a new approach for 'Gutenberg and Decoupled Applications'](https://graphql-api.com/blog/proposing-a-new-approach-for-gutenberg-and-decoupled-applications/) (the `Block` type will be added in an upcoming version of the plugin):
