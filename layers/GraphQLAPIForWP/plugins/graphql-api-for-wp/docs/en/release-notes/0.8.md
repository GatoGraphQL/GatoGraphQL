...
It is used on the newly-introduced `metaValue`, `metaValues` and `option` fields, to acknowledge the fact that we do not know in advance the type of the data returned by these fields.
