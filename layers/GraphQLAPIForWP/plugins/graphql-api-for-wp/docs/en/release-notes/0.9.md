...
...added input field for filtering the results:
...
### Updating GraphQL queries to use the new `filter` field argument

Since replacing field arguments with `filter`, all GraphQL queries must be updated.
...
Please visualize the new GraphQL schema (in the Explorer Docs in GraphiQL, and in the Interactive Schema) and update the GraphQL queries accordingly.
