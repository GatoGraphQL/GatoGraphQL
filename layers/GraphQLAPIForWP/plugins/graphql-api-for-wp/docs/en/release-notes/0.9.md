...
The plugin now validates that the GraphQL query does not contain cyclical fragment references, as [specified by the GraphQL spec](https://spec.graphql.org/draft/#sec-Fragment-spreads-must-not-form-cycles).
