...
For instance, the field `Root.loginUser` now receives a field argument `credentials`, which is a oneof input object allowing to provide several methods to log into the website. Currently it accepts only 1 method, via the named member `website` (which accepts the `usernameOrEmail` and `password` entries), and other methods can be added in the future (such as `jwt` and `applicationPasswords`):
...
<a href="../../images/releases/v09/settings-treat-user-email-as-private-data.png" target="_blank">![Broken entries in Access Control List](../../images/releases/v09/settings-treat-user-email-as-private-data.png)</a>
