...
All `date` fields (such as `Post.date`, `Media.date` and `Comment.date`) and `modified` fields are now of type `DateTime`.

The previous fields, with `String` type, have been renamed to `dateAsString` and `modifiedAsString`. These fields have kept the argument `format`, to specify how to print the date value.
