...
Fields to fetch a single entity, such as `Root.post` or `Root.user`, used to receive argument `id` to select the entity. Now they have been expanded: `id` has been replaced with argument `by`, which is a oneof input filter (explained later on) to query the entity by different properties.
...
In all fields retrieving a list of elements, such as `Root.posts` or `Root.posts`, a new argument `filter` now allows us to filter the results. The filtering values are customized for each field, via a corresponding InputObject (see later on).
...
All fields retrieving a list of elements can be paginated and sorted using customized InputObjects, and these are always placed under field arguments `pagination` and `sort`.
...
Added arguments `roles` and `excludeRoles` to filter by user roles.
...
Support for custom scalars has been added to the GraphQL server! 🎉

Custom scalars allow you to better represent your data, whether for getting an input via a field argument, or printing an output in the response.

(The documentation on how to create custom scalars is not ready yet... an example implementation is `Email`, [here](https://github.com/leoloso/PoP/blob/a882ddf1300ee915b96683fbdf56f09be2ea0447/layers/Schema/packages/schema-commons/src/TypeResolvers/ScalarType/EmailScalarTypeResolver.php)).

### Implementation of standard custom scalar types

Several standard custom scalar types have been implemented, and ready to be referenced:
...
Similar to custom scalars, custom enum types can be added to the GraphQL schema! 🚀

(The documentation on how to create custom enums is not ready yet... an example implementation is `CommentStatus`, [here](https://github.com/leoloso/PoP/blob/c320cb1a4e5db48c5045cb37b66506b4a4a9a695/layers/Schema/packages/comments/src/TypeResolvers/EnumType/CommentStatusEnumTypeResolver.php)).
...
For instance, the field `Root.loginUser` now receives a field argument `by`, which is a oneof input object allowing to provide several methods to log into the website. Currently it accepts only 1 method, via the named member `credentials` (which accepts the `usernameOrEmail` and `password` entries), and other methods can be added in the future (such as `jwt` and `applicationPasswords`):
...
This way, `loginUser` can tackle all methods of logging the user, which makes the GraphQL schema leaner and more elegant (otherwise, we would need fields `loginUserByCredentials`, `loginUserByJWT` and `loginUserByApplicationPassword`).
...
- The "admin" elements (such as `User.roles`) are not added to the schema
...
Fields to fetch a single entity, such as `Root.post` or `Root.user`, used to receive argument `id` to select the entity. Now they have been expanded: `id` has been replaced with argument `by`, which is a oneof input filter to query the entity by different properties.
...
All date fields (including `Post.date`, `Media.date`, `Comment.date`, `Post.modified`, `Media.modified` and `User.registered`) were of type `String`. These fields have now been converted to type `DateTime`, and have had the argument `format` removed (since specifing how to print the date value does not apply anymore).

The previous fields have been renamed as `...AsString` (such as `Post.dateAsString`, `Media.modifiedAsString` and `User.registeredAsString`), keeping the type `String` and argument `format`.

### Updated the arguments for mutation `loginUser`

Logging-in the user was done like this:
...
Since v0.9, `loginUser` relies on the oneof input object, and logging-in the user must be done like this:
...
### Converged the "admin" and non-admin fields

Removed all the "unrestricted" fields. From `v0.9` onwards, their functionality is provided by the corresponding standard field, which will have additional inputs whenever the Schema Configuration has `Schema Expose Admin Data` property enabled.

For instance, field `Root.posts` has argument `filter`. When `Schema Expose Admin Data` property is enabled, this argument (of type `RootPostsFilterInput`) exposes an additional input `status`, enabling to filter posts by status `"draft"` (which is private data).

The following "admin" (or "unrestricted") fields were removed, and are now handled by:
...
From now on, fields `CustomPost.status` and `User.email` are treated as private data. As such, they are exposed only if property `Expose Admin Data` is enabled.
