...
abstract class AbstractEndpointResolverAppStateProvider extends AbstractAppStateProvider
{
    ...
    public function consolidate(array &$state): void
    {
        ...
        list(
            $graphQLQuery,
            $graphQLVariables
        ) = $this->getEndpointResolver()->getGraphQLQueryAndVariables(App::getState(['routing', 'queried-object']));
        if (!$graphQLQuery) {
         ...
         */
        $graphQLVariables ??= [];
        $state['variables'] = $this->getEndpointResolver()->doURLParamsOverrideGraphQLVariables(App::getState(['routing', 'queried-object'])) ?
            array_merge(
                $graphQLVariables,
                $state['variables'] ?? []
            ) :
