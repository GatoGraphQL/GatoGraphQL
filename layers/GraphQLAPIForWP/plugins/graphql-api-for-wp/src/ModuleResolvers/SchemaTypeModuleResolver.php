...
class SchemaTypeModuleResolver extends AbstractModuleResolver
{
    ...
    {
        return match ($module) {
            self::SCHEMA_GENERIC_CUSTOMPOSTS => \__('Generic Custom Posts', 'graphql-api'),
    ...
    public function getSettings(string $module): array
    {
        ...
        $sensitiveDataDescPlaceholder = \__('If checked, the <strong>%s</strong> data is exposed in the schema (whether as an object field for querying, or as an input field for filtering) only if the Schema Configuration has option <code>Expose Sensitive Data in the Schema</code> enabled (i.e. the data is for private use only); otherwise, the data is always exposed in the schema (i.e. it is public)', 'graphql-api');
