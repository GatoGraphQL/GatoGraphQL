...
<tr><td><a href="docs/modules/schema-expose-admin-data/en.md">Expose Admin Data in the Schema</a></td><td>Expose "sensitive" data elements in the GraphQL schema (such as field <code>Root.roles</code>, input field <code>Root.posts(status:)</code>, and others), which provide access to potentially private user data</td></tr>
