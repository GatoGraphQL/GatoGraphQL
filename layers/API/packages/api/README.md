...
In the example below, directive `<forEach>` iterates all the elements from an array, and passes each of them to directive `<applyField>` which executes field `arrayJoin` on them:
...
<a href="https://newapi.getpop.org/api/graphql/?query=echo(%5B%5Bbanana,apple%5D,%5Bstrawberry,grape,melon%5D%5D)@fruitJoin%3CforEach%3CapplyField(function:arrayJoin,addArguments:%5Barray:%{value}%,separator:%22---%22%5D)%3E%3E">View query results</a>
