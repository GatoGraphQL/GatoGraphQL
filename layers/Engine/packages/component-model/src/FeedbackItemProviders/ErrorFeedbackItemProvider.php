...
class ErrorFeedbackItemProvider extends AbstractFeedbackItemProvider
{
    ...
    public function getMessagePlaceholder(string $code): string
    {
        return match ($code) {
            ...
            self::E9 => $this->__('Data for object of type \'%s\' and ID \'%s\' cannot be loaded (it\'s likely that either the DataLoader or the TypeResolver has issues)', 'component-model'),
            self::E10 => $this->__('In union type \'%s\', data for object with ID \'%s\' cannot be loaded (it\'s likely that the DataLoader has issues, or that there is no target TypeResolver for the object)', 'component-model'),
