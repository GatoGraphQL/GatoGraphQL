...
final class ResolveValueAndMergeDirectiveResolver extends AbstractGlobalDirectiveResolver implements MandatoryDirectiveServiceTagInterface
{
    ...
    public function resolveDirective(
        ...
        if ($resultIDItems) {
            $this->resolveValueForResultItems($objectTypeResolver, $resultIDItems, $idsDataFields, $dbItems, $previousDBItems, $variables, $messages, $dbErrors, $dbWarnings, $dbDeprecations, $schemaErrors, $schemaWarnings, $schemaDeprecations);
        }
    }
    ...
    protected function resolveValueForResultItems(ObjectTypeResolverInterface $objectTypeResolver, array &$resultIDItems, array &$idsDataFields, array &$dbItems, array &$previousDBItems, array &$variables, array &$messages, array &$dbErrors, array &$dbWarnings, array &$dbDeprecations, array &$schemaErrors, array &$schemaWarnings, array &$schemaDeprecations)
    {
        ...
        foreach (array_keys($idsDataFields) as $id) {
            ...
            $this->resolveValuesForResultItem($objectTypeResolver, $id, $resultItem, $idsDataFields[(string)$id]['direct'], $dbItems, $previousDBItems, $variables, $expressions, $dbErrors, $dbWarnings, $dbDeprecations);
            ...
            foreach (array_filter($idsDataFields[$id]['conditional']) as $conditionDataField => $conditionalDataFields) {
                ...
                $conditionFieldOutputKey = $this->fieldQueryInterpreter->getUniqueFieldOutputKey($objectTypeResolver, $conditionDataField);
        ...
        if ($enqueueFillingResultItemsFromIDs) {
            $objectTypeResolver->enqueueFillingResultItemsFromIDs($enqueueFillingResultItemsFromIDs);
        }
    }
    ...
    protected function resolveValuesForResultItem(
        ...
        foreach ($dataFields as $field) {
            $this->resolveValueForResultItem($objectTypeResolver, $id, $resultItem, $field, $dbItems, $previousDBItems, $variables, $expressions, $dbErrors, $dbWarnings, $dbDeprecations);
        }
    }
    ...
    protected function resolveValueForResultItem(
        ...
        $value = $this->resolveFieldValue($objectTypeResolver, $id, $resultItem, $field, $previousDBItems, $variables, $expressions, $dbWarnings, $dbDeprecations);
        $this->addValueForResultItem($objectTypeResolver, $id, $field, $value, $dbItems, $dbErrors);
    }
    ...
    protected function resolveFieldValue(
        ...
        $value = $objectTypeResolver->resolveValue($resultItem, $field, $variables, $expressions);
    ...
    protected function addValueForResultItem(ObjectTypeResolverInterface $objectTypeResolver, $id, string $field, $value, array &$dbItems, array &$dbErrors)
    {
