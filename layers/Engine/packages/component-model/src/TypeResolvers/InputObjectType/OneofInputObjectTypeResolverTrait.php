    ...
    protected function validateOneofInputObjectValue(stdClass $inputValue): ?Error
    {
        ...
        if ($inputValueSize > 1) {
            return new Error(
                $this->getErrorCode(),
                sprintf(
                    $this->__('The oneof input object \'%s\' must receive exactly 1 input, but \'%s\' inputs were provided (\'%s\')', 'component-model'),
        ...
        if ($inputValueSize === 0 && $this->isOneInputValueMandatory()) {
            return new Error(
                $this->getErrorCode(),
                sprintf(
                    $this->__('No input value was provided to the oneof input object \'%s\'', 'component-model'),
                    $this->getMaybeNamespacedTypeName()
                )
            );
        }
