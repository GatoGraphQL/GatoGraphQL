...
abstract class AbstractApplyNestedDirectivesOnArrayItemsDirectiveResolver extends AbstractGlobalDirectiveResolver
{
    ...
    public function resolveDirective(
        ...
        foreach ($idsDataFields as $id => $dataFields) {
            foreach ($dataFields['direct'] as $field) {
                $fieldOutputKey = $this->fieldQueryInterpreter->getUniqueFieldOutputKey($objectTypeResolver, $field);
                ...
                $array = $value;
                if ($arrayItems = $this->getArrayItems($array, $id, $field, $objectTypeResolver, $resultIDItems, $dbItems, $previousDBItems, $variables, $messages, $dbErrors, $dbWarnings, $dbDeprecations)) {
                    $execute = true;
                    foreach ($arrayItems as $key => &$value) {
                        ...
                        $arrayItemPropertyOutputKey = $this->fieldQueryInterpreter->getUniqueFieldOutputKey($objectTypeResolver, $arrayItemProperty);
                    ...
                    $this->addExpressionsForResultItem($objectTypeResolver, $id, $field, $resultIDItems, $dbItems, $previousDBItems, $variables, $messages, $dbErrors, $dbWarnings, $dbDeprecations, $schemaErrors, $schemaWarnings, $schemaDeprecations);
                }
            }
        }
        ...
        if ($execute) {
            ...
            foreach ($idsDataFields as $id => $dataFields) {
                foreach ($dataFields['direct'] as $field) {
                    ...
                    $array = $value;
                    $arrayItems = $this->getArrayItems($array, $id, $field, $objectTypeResolver, $resultIDItems, $dbItems, $previousDBItems, $variables, $messages, $arrayItemDBErrors, $arrayItemDBWarnings, $arrayItemDBDeprecations);
                    // The value is an array. Unpack all the elements into their own property
                    foreach ($arrayItems as $key => &$value) {
                        $arrayItemAlias = $this->createPropertyForArrayItem($fieldAlias ? $fieldAlias : QuerySyntax::SYMBOL_FIELDALIAS_PREFIX . $fieldName, (string) $key);
                        $arrayItemProperty = $this->fieldQueryInterpreter->composeField(
                        ...
                        $this->addProcessedItemBackToDBItems($objectTypeResolver, $dbItems, $dbErrors, $dbWarnings, $dbDeprecations, $dbNotices, $dbTraces, $id, $fieldOutputKey, $key, $arrayItemValue);
                    }
                }
            }
        }
     ...
     */
    abstract protected function getArrayItems(array &$array, int | string $id, string $field, ObjectTypeResolverInterface $objectTypeResolver, array &$resultIDItems, array &$dbItems, array &$previousDBItems, array &$variables, array &$messages, array &$dbErrors, array &$dbWarnings, array &$dbDeprecations): ?array;
        ...
        if ($addExpressions || $appendExpressions) {
            ...
            foreach ($addExpressions as $key => $value) {
                ...
                if ($this->fieldQueryInterpreter->isFieldArgumentValueAField($value)) {
                    $resolvedValue = $objectTypeResolver->resolveValue($resultIDItems[(string)$id], $value, $variables, $expressions, $options);
