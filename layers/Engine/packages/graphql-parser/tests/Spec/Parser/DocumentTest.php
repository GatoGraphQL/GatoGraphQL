...
class DocumentTest extends AbstractTestCase
{
    ...
    public function testMissingFragmentReferencedByFragment()
    {
        ...
        $this->expectExceptionMessage($this->getFeedbackMessageProvider()->getMessage(FeedbackMessageProvider::E_5_5_2_1, 'F1'));
        $parser = $this->getParser();
        $document = $parser->parse('
            query StarWarsAppHomeRoute($names_0:[String!]!, $query: String) {
              factions(names:$names_0, test: $query) {
    ...
    {
        $this->expectException(InvalidRequestException::class);
        $this->expectExceptionMessage($this->getFeedbackMessageProvider()->getMessage(FeedbackMessageProvider::E_5_5_2_1, 'F2'));
        $parser = $this->getParser();
        $document = $parser->parse('
            query StarWarsAppHomeRoute($names_0:[String!]!, $query: String) {
              factions(names:$names_0, test: $query) {
    ...
    {
        $this->expectException(InvalidRequestException::class);
        $this->expectExceptionMessage($this->getFeedbackMessageProvider()->getMessage(FeedbackMessageProvider::E_5_2_1_1, 'SomeQuery'));
        $parser = $this->getParser();
        $document = $parser->parse('
            query SomeQuery {
                users {
