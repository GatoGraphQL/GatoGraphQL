...
The [“WordPress Content Translation” Bundle](https://gatographql.com/bundles/wordpress-content-translation) gives you all the tools needed to create a GraphQL query that extracts text properties from Gutenberg blocks, translates them to another language using the Google Translate API, and then stores the post's content back to the database.
