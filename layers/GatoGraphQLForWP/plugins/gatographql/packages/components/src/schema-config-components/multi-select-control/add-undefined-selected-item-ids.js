...
const AddUndefinedSelectedItemIDs = withSelect(
	( select, { items, selectedItems } ) => {
		...
		const undefinedSelectedItemIDs = selectedItems.filter(
			( selectedItemID ) => ! itemValues.includes( selectedItemID )
		);
		return {
			items: items.concat(
				undefinedSelectedItemIDs.map( ( undefinedSelectedItemID ) => ( {
					group: __( 'Undefined elements', 'gatographql' ),
					groupKind: 'undefined',
					title: getLabelForNotFoundElement(
