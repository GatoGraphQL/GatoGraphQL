...
function getTypeFieldsKey( graphQLVariables ) {
	return (
		graphQLVariables || parameters.typeFields.defaultVariables
	).toString();
}
...
export function getTypeFields(
	state,
	graphQLVariables,
	keepScalarTypes = false,
	keepIntrospectionTypes = false
) {
	const key = getTypeFieldsKey( graphQLVariables );
	...
	if ( ! keepIntrospectionTypes ) {
		typeFields = typeFields.filter(
			( element ) => ! element.typeName.startsWith( '__' )
		);
	}
...
function getGlobalFieldsKey( graphQLVariables ) {
	return (
		graphQLVariables || parameters.globalFields.defaultVariables
	).toString();
}
 ...
 * @return {string|null} Error message, if any.
 */
export function getRetrievingGlobalFieldsErrorMessage(
	state,
	graphQLVariables
) {
	const key = getGlobalFieldsKey( graphQLVariables );
...
function getDirectivesKey( graphQLVariables ) {
	return (
		graphQLVariables || parameters.directives.defaultVariables
	).toString();
}
