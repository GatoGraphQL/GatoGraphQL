...
const PersistedQueryEndpointGuide = ( props ) => {
	return (
		<Guide { ...props } >
            <GuidePage>
                <h1>{ __('Creating Persisted Queries', 'gatographql') }</h1>
                <p>{ __('This tutorial video demonstrates how to create a new GraphQL persisted query:', 'gatographql') }</p>
                ...
                { __('Your browser does not support the video tag.', 'gatographql') }
                </video> 
                <p>
                    <ExternalLink
                        href="https://vimeo.com/413503547"
                    >
                        { __('Watch in Vimeo', 'gatographql') }
                    </ExternalLink>
                </p>
            </GuidePage>
            <GuidePage>
                <h1>{ __('GraphiQL client', 'gatographql') }</h1>
...
`{
  field(arg: "value") {
    subField
            ...
            <GuidePage>
                <h1>{ __('Persisted Query Options', 'gatographql') }</h1>
                <h2>{ __('Enabled', 'gatographql') }</h2>
                ...
                <h2>{ __('Schema configuration', 'gatographql') }</h2>
                ...
                <h2>{ __('Accept variables as URL params?', 'gatographql') }</h2>
	...
	return (
		<>
			<Button isTertiary onClick={ openGuide }>
				{ __('Open Guide: “Creating Persisted Queries”', 'gatographql') }
			</Button>
			{ isOpen && (
