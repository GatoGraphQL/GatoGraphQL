 ...
 */
__webpack_public_path__ = window.gatographqlPersistedQueryEndpointOptions?.publicPath;
...
const getMarkdownContent = ( fileName, lang ) => {
	return import( /* webpackChunkName: "componentDocs/[request]" */ `@componentDocs/${ fileName }/${ lang }.md` )
	...
	// return import( /* webpackMode: "eager" */ `@componentDocs/${ fileName }/${ lang }.md` )
	// 	.then(obj => obj.default)
	// ---------------------------------------------
}
...
const getMarkdownContentOrUseDefault = ( fileName, defaultLang, lang ) => {
	lang = lang || window.gatographqlPersistedQueryEndpointOptions?.localeLang
	defaultLang = defaultLang || window.gatographqlPersistedQueryEndpointOptions?.defaultLang
