...
`_sendEmail` is a global field (or, more precisely, a global mutation). This means that, if **Nested Mutations** are enabled, this mutation can be executed on any type from the GraphQL schema (i.e. not only in `MutationRoot`).
